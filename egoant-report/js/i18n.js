...
(function () {
  const I18N = {
    zh: {
      "walk.th.sel": "候选判别器",
      "world.term.e2e.neighbor": "邻段重标：给当前段时同时给上一段 / 当前段 / 下一段的帧。变体包括：种子邻段重标（附带一句种子描述作提示）、原始帧先验邻段重标（把原始帧重标结果当作文本先验）。看似能提供上下文，但常把邻段动作写进当前句，从而拉低准确率。",
      ...
      "story.e2e.5": "S2 边界 + 邻段重标 · 27B / 397B：提交邻段视觉上下文。27B 侧试过种子邻段重标与原始帧先验邻段重标；397B 侧用原始帧描述作文本先验。",
      ...
      "cost.row.wgo_scope": "原始帧 / 候选判别器工程估计 + 产物 API 次数",
    ...
    en: {
      ...
      "world.term.e2e.neighbor": "Neighbor relabel: the labeler also sees previous / current / next segment frames. Variants include seeded-neighbor relabel (with a seed description as a hint) and raw-prior neighbor relabel (using a raw-frame relabel result as the text prior). This looks like useful context, but neighboring actions often leak into the current sentence and hurt accuracy.",
      ...
      "story.e2e.5": "S2 bounds + neighbor relabel · 27B / 397B: submit neighbor visual context. 27B tries seeded-neighbor and raw-prior neighbor; 397B uses a raw-frame description as the text prior.",
      ...
      "cost.row.wgo_scope": "raw-frame / candidate-selector estimate + artifact API counts",
