...
(function () {
  const I18N = {
    zh: {
      "cost.simple.compare": "读表：推荐路径得分 0.1542，已接近 Macrodata 闭源最优 0.168；费用约 <strong>$2.7 / 视频小时</strong>，与其公开批量 <strong>$2.64 / 视频小时</strong> 同量级，但可私有部署。成本更敏感时，可退回单路原始帧（约 <strong>$1.1 / 视频小时</strong>）。",
      "cost.simple.details": "<li>EgoANT 两条路径评在 HomER 25 集（约 40 分钟，预测 308 段）；Macrodata 得分为全量约 100 集上的闭源最优，费用来自其公开批量账单。</li><li>单路：每段 1 次标注调用。多候选：每段约 4 路候选描述 + 1 次候选判别，调用量大约是单路的 4–5 倍，所以更贵。</li><li>美元按 Qwen 公开单价估算；token 有上下浮动，表中取偏低一侧的点估计（约在区间下三分之一处），不是上界。</li>",
      ...
      "pipeline.e2e.analysis": "<p>结合上图与表，可以归纳如下。</p><ol><li>腕速一体基线：预测 810 段 / 参考 470 段，端到端整流程得分 0.0641，明显低于后续固定 S2 边界的路径。</li><li>分段模型自标：在同一 S2 边界上沿用分段描述，得分为 0.1234，说明只换更好描述路径仍有空间。</li><li>原始帧重标 · 27B / 397B：默认原始帧重标下，27B 为 0.1285，段内缩进 0.5 秒为 0.1337；397B 为 0.1414。同一方法下 397B 高于 27B，但增益主要来自噪声预测边界上的定稿，而不是固定参考边界上的固定分段标注得分优势。</li><li>ffmpeg 抽帧重标 · 27B / 397B：27B 为 0.1440，397B 为 0.1491。换抽帧实现本身就能抬分，且两边同向；该路径常作为候选判别器的候选来源，而不是单独的新策略。</li><li>邻段重标 · 27B / 397B：27B 种子邻段重标 0.1260、原始帧先验邻段重标 0.1414；397B 先验邻段重标 0.1491。邻段上下文有时接近单路原始帧 / ffmpeg，但不稳定，且容易把邻段动作写进当前句。</li><li>多候选判别 · 27B / 397B：27B 候选判别器为 0.1362，低于同组最好的 27B ffmpeg；397B 候选判别器为 <strong>0.1542</strong>，语义匹配 60，仍为本组最高。WGO-Bench 公开的闭源最优端到端整流程得分为 0.168（全量约 100 集）；我们在 HomER 子集上的 0.1542 已与之接近。成本敏感场景可走单路原始帧 / ffmpeg；高精度场景保留 397B 候选判别器。</li></ol>",
      ...
      "story.e2e.6": "S2 边界 + 多候选判别 · 27B / 397B：在同一 S2 边界上生成多路候选描述，再分别由 27B 与 397B 候选判别器定稿。重标提示词见 <a href=\"#label-prompt\">3.2</a>；候选判别器提示词见下方折叠。",
      ...
      "tldr.pt5": "<strong>单路原始帧标注已接近多候选判别方案，且成本更低。</strong>在相同预测边界上，使用 Qwen3.5-397B-A17B 进行单路原始帧标注，端到端整流程得分为 <strong>0.1414</strong>；生成多条描述并由候选判别器定稿后，得分为 <strong>0.1542</strong>，相对提高约 9.1%。前者约 <strong>$1.1 / 视频小时</strong>，后者约 <strong>$2.7 / 视频小时</strong>。对成本敏感的批量处理，单路原始帧标注是更合适的默认方案。",
      ...
      "cost.note": "分段边界固定后，定稿有两种做法：每段只写一次描述，或先写多路候选再挑一句。下表把这两条开放权重路径，与 Macrodata 公开的闭源端到端批量数字放在一起对照。",
      ...
      "story.e2e.p": "固定 S2 分段边界后，只改变语义标注路径。<strong>27B 自标</strong>为 0.1234；<strong>27B 原始帧 · 段内缩进 0.5 秒</strong>为 0.1337；<strong>27B ffmpeg 抽帧</strong>为 0.1440；<strong>27B 候选判别器</strong>为 0.1362。也就是说，27B 在固定参考边界的固定分段标注得分上最高，但在噪声预测边界上的定稿仍弱于 397B 候选判别器。<strong>397B 原始帧重标</strong>为 0.1414；<strong>397B ffmpeg 抽帧</strong>和<strong>397B 先验邻段重标</strong>均为 0.1491；<strong>397B 候选判别器</strong>从多路候选中选择最终标签，取得最高观察值 <strong>0.1542</strong>。该增益应与额外候选生成和候选判别器调用成本一起理解。",
      "story.e2e.fold": "展开：E2E 实验细节（做法卡）",
      "story.takeaway": "<strong>要点：</strong>分片请求边界会引入伪边界；分段规则定义要切分的完成动作，局部精修决定边界粒度；三种规则合并策略均降低视频分段得分；Gemini 重评后，原始帧 · 27B 是固定参考边界下观察值最高的标注配置，但端到端最高观察值仍来自 S2 预测边界 + 397B 多候选判别器。换句话说，<strong>27B 是最佳固定边界标注器，但不是最佳噪声预测片段定稿器</strong>。视觉提示叠加、邻段拼贴和近似手部拼贴（含 27B 补评）目前不作为默认路径。",
      "th.cond": "条件",
      "th.model": "模型或方法",
      "th.note": "结果一句话",
      "th.acc": "固定分段标注得分",
    ...
    en: {
      "cost.simple.compare": "Reading the table: the recommended path scores 0.1542, close to Macrodata’s closed-source best of 0.168; cost is about <strong>$2.7 / video-hour</strong>, in the same ballpark as their public batch bill of <strong>$2.64 / video-hour</strong>, but privately deployable. For tighter budgets, fall back to single-path raw frames (about <strong>$1.1 / video-hour</strong>).",
      "cost.simple.details": "<li>The two EgoANT paths are scored on HomER 25 episodes (~40 minutes, 308 predicted segments). Macrodata’s score is the closed-source best on the full ~100-episode set; its dollar figure comes from their public batch bill.</li><li>Single-path: one labeling call per segment. Multi-candidate: about four candidate descriptions plus one selector call per segment—roughly 4–5× the labeling calls, hence the higher cost.</li><li>Dollars use public Qwen unit prices. Tokens swing within a range; the table reports a low-side point estimate (~one-third up from the floor), not the upper bound.</li>",
      ...
      "story.e2e.6": "S2 bounds + multi-candidate select · 27B / 397B: generate multiple candidate descriptions on the same S2 bounds, then finalize with 27B and 397B candidate selectors. Relabel prompts are in <a href=\"#label-prompt\">§3.2</a>; the candidate-selector prompt is in the fold below.",
      ...
      "cost.note": "With segmentation fixed, final labels can follow two paths: write one description per segment, or generate multiple candidates and pick one. The table below puts those two open-weight paths next to Macrodata’s published closed-source end-to-end batch numbers.",
      ...
      "walk.s5.t": "Candidate-selector finalization",
