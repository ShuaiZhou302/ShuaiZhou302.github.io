...
(function () {
  const I18N = {
    zh: {
      ...
      "story.seg.7": "S2 + 相邻片段规则合并：这是对最优 S2 预测片段列表的离线后处理，不是新的模型调用。输入是最优 S2 的预测片段，输出是合并后的片段列表；脚本按时间顺序只比较相邻段。三种规则分别是：标签规范化后完全相同则合并；主要动词/物体相同则合并；先桥接很短的时间空隙，再按前两类规则尝试合并。",
      "pipeline.merge.fold": "相邻片段规则合并伪代码",
      "pipeline.merge.note": "<p><strong>读码说明：</strong></p><ul><li><code>compatible(...)</code> 决定相邻两段能不能并成一段；三种 <code>strategy</code> 对应正文里的三条规则。</li><li><code>normalize</code> 会去掉大小写、空白等表面差异后再比标签；不是语义相似度。</li><li><code>choose_representative_label</code> 合并后只保留一句代表标签，通常取较长或信息更完整的那句。</li><li><code>bridge_short_gap</code> 里的 <code>small_gap</code> 只把很短的时间空隙当成仍可合并，不会跨过大空洞。</li><li>脚本只读 <code>start_sec</code> / <code>end_sec</code> / <code>subtask</code>：合并时只能删掉或挪动已有边界，不能发明新切点。</li></ul>",
    ...
    en: {
      ...
      "story.seg.7": "S2 + adjacent-segment rule merges: an offline postprocess over the best S2 predicted segment list, not a new model call. The input is the best S2 predictions and the output is a merged list; the script only compares time-sorted neighbors. Three rules: merge after label normalization when identical; merge when the main verb/object agrees; or first bridge very short gaps, then apply the previous compatibility checks.",
      "pipeline.merge.fold": "Adjacent-segment rule-merge pseudocode",
      "pipeline.merge.note": "<p><strong>How to read the code:</strong></p><ul><li><code>compatible(...)</code> decides whether two neighbors can become one span; the three <code>strategy</code> values match the rules in the body text.</li><li><code>normalize</code> strips surface differences such as case and whitespace before comparing labels; it is not a semantic similarity check.</li><li><code>choose_representative_label</code> keeps one label after a merge, usually the longer or more informative string.</li><li>In <code>bridge_short_gap</code>, <code>small_gap</code> only treats very short temporal holes as still mergeable; it will not bridge large gaps.</li><li>The script reads only <code>start_sec</code> / <code>end_sec</code> / <code>subtask</code>: merges can delete or move existing boundaries, but cannot invent new cuts.</li></ul>",
