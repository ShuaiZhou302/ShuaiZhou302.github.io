...
(function () {
  const I18N = {
    zh: {
      ...
      "walk.s0.t": "原始视频",
      "walk.s0.p": "这是本样本的原始视频。",
      "walk.s1.t": "生成拼贴图",
      "walk.s1.p": "参数与上文拼贴图设定一致。模型后续只看这些带时间戳的拼贴图，而不是原始 MP4 字节流。",
      "walk.s1.note": "下方为首张拼贴图示例（同参数）；整集还会生成多张同类拼贴图。",
      "walk.s2.t": "粗分：整集一次 + 切段规则清单",
      "walk.s2.p": "把该集尽量多的<strong>拼贴图</strong>一次送给分段模型，并在请求文本里附上<strong>切段规则清单</strong>，得到整集粗分边界。",
      "walk.s3.t": "局部精修：窗口不外扩，盖住完整动作",
      "walk.s3.p": "在粗边界附近开局部时间窗，用<strong>同一版式</strong>的拼贴图再切一次。<strong>窗口不外扩</strong>：只看粗段内部；<strong>盖住完整动作</strong>：窗内看得见起止的完成操作都要切出。以本集为例，整集粗分在约 <strong>26.0s</strong> 处得到「擦抽屉内部」粗段（约 <strong>[26.0, 42.0]</strong>）；下图是该粗段内一张局部拼贴图，覆盖 <strong>26.5s–36.0s</strong>（与上一步整集开头的拼贴图不同）。",
    ...
    en: {
      ...
      "walk.s0.p": "This is the original video for this sample.",
      ...
      "walk.s2.p": "Submit as many <strong>contact sheets</strong> from the episode as possible in one call, with a <strong>segmentation-rule list</strong> in the request text, to get whole-episode coarse boundaries.",
      "walk.s3.t": "Local refinement: no pad-out, cover full actions",
      "walk.s3.p": "Open a local time window near the coarse bounds and re-cut with the <strong>same collage layout</strong>. <strong>No pad-out</strong>: look only inside the coarse segment. <strong>Cover full actions</strong>: cut every completed action whose start and end are visible in the window. In this episode, whole-episode coarse cut places a \"wipe drawer interior\" coarse segment near <strong>26.0s</strong> (about <strong>[26.0, 42.0]</strong>); the figure below is one in-window collage covering <strong>26.5s–36.0s</strong> (different from the episode-start collage above).",
