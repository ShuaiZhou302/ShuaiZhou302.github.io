...
(function () {
  const I18N = {
    zh: {
      ...
      "pipeline.seg.analysis": "<p>结合上图与表可见，多种不同方法呈现了不同的表现：</p><ol><li>腕速规则切段并合并：预测片段远多于参考分段（810 vs 470），视频分段 F1 仅 0.0953，说明运动低谷信号难以对齐动作语义边界。</li><li>拼贴图分片：请求接缝容易被当成动作边界。同设置下 397B 为 0.0952、27B 为 0.1278；分片本身限制了双方得分。</li><li>整集一次提交（无切段规则清单）：去掉分片接缝后伪边界减少，但预测片段偏少（148），F1 为 0.1230。</li><li>整集一次提交 + 切段规则清单：F1 升至 0.1369，比无规则清单更好，整体仍偏保守。</li><li>S1 加密切分：参考找回率上升（配对成功 80），F1 到 0.1556，同时预测段数增至 558，切分更细。</li><li>S2 局部精修：是主要增益来源。窗外扩约 1 秒为 0.1674；窗口不外扩但尚未要求盖住完整动作时为 0.1711，窗外扩 0.5/1/2 秒均更低；算法补覆盖为 0.1635，不及把「盖住完整动作」写进提示词。最终「窗口不外扩并盖住完整动作」取得最高视频分段 F1 0.2031。</li><li>相邻片段规则合并：三种策略都基于上述最优 S2 预测做后处理，F1 分别降为 0.1987 / 0.1947 / 0.1883，因此不作为默认配置。</li><li>模型对比：同一拼贴图分片设置下，较新的 Qwen3.6-27B（0.1278）高于 Qwen3.5-397B（0.0952）；最终最优 S2 配置也使用 27B。说明在本组实验里，更新、更先进的模型表现更好。</li></ol>",
      ...
      "story.seg.6": "S2 局部精修：先由整集粗分得到粗边界，再围绕每个粗边界开局部时间窗，把窗内拼贴图交回模型重切。得分表中的变体包括：窗外扩约 1 秒、窗口不外扩（尚未盖住完整动作）、窗外扩 0.5/1/2 秒、窗口不外扩 + 算法补覆盖，以及窗口不外扩并盖住完整动作（精修提示词见 <a href=\"#app-prompts\">附录 F</a> · <a href=\"prompts/s2_fullcover_refine.md\" download>下载</a>）。",
      "story.seg.7": "相邻片段规则合并：在 S2「窗口不外扩并盖住完整动作」的预测结果上，用规则脚本按时间相邻、标签完全相同、动词/物体相同或短间隙桥接等策略合并片段，不再调用模型。",
    ...
    en: {
      ...
      "story.seg.6": "S2 local refinement: take coarse bounds from the whole-episode pass, open a local time window around each bound, and re-cut from the in-window contact sheets. Score-table variants include ≈1s pad-out, no pad-out without cover-full-actions, 0.5/1/2s pad-out, no pad-out + algorithmic cover, and no pad-out + cover full actions (refine prompt: <a href=\"#app-prompts\">Appendix F</a> · <a href=\"prompts/s2_fullcover_refine.md\" download>download</a>).",
      "story.seg.7": "Adjacent-segment rule merges: starting from the S2 “no pad-out + cover full actions” predictions, merge with rule scripts based on temporal adjacency, identical labels, shared verb/object, or short-gap bridging, without another model call.",
