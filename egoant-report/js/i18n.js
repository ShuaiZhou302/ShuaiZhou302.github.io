...
(function () {
  const I18N = {
    zh: {
      ...
      "tldr.pt5": "<strong>单路原始帧标注已接近多候选判别方案，且成本更低。</strong>在相同预测边界上，使用 Qwen3.5-397B-A17B 进行单路原始帧标注，端到端整流程得分为 <strong>0.1414</strong>；生成多条描述并由候选判别器定稿后，得分为 <strong>0.1542</strong>，相对提高约 9.1%。前者约 <strong>$1.2 / 视频小时</strong>，后者约 <strong>$3.0 / 视频小时</strong>。对成本敏感的批量处理，单路原始帧标注是更合适的默认方案。",
      ...
      "cost.note": "分段边界固定后，定稿有两种做法：每段只写一次描述，或先写多路候选再挑一句。下表按 HomER 25 集（约 40 分钟）估算开放权重模型费用，并折算成每视频小时。",
    ...
    en: {
      ...
      "cost.simple.raw": "Single-path raw-frame labeling",
      ...
      "tldr.pt5": "<strong>Single-pass raw-frame labeling is close to multi-candidate selection at substantially lower cost.</strong> On the same predicted boundaries, a single Qwen3.5-397B-A17B raw-frame pass reaches E2E F1 <strong>0.1414</strong>. Generating multiple descriptions and using a candidate selector reaches <strong>0.1542</strong>, a 9.1% relative gain. Estimated costs are about <strong>$1.2 / video-hour</strong> and <strong>$3.0 / video-hour</strong>, respectively. For cost-sensitive batch processing, the single-pass path is the more practical default.",
      ...
      "cost.note": "With segmentation fixed, final labels can follow two paths: write one description per segment, or generate multiple candidates and pick one. The table below estimates open-weight model cost on HomER 25 episodes (~40 minutes), converted to per video-hour.",
