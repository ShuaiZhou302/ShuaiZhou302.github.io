...
(function () {
  const I18N = {
    zh: {
      ...
      "metrics.lead": "三种分数依次衡量：时间边界是否准确、给定边界后的描述是否正确，以及端到端分段与标注的整体表现。公式汇总见 <a href=\"#app-metrics\">附录 A</a>。",
      "metrics.li1": "Segment F1（分段得分）：只评时间边界。对一对预测片段与参考片段，时间 IoU 等于「两段重叠的时长 ÷ 两段合起来覆盖的总时长」；完全重合时为 1，完全不重合时为 0。IoU≥0.75 的片段对才有资格形成一对一时间匹配。",
      "metrics.li2": "Label Acc（固定分段标注得分）：直接使用人工边界，不让模型预测时间，只判断每个片段的描述是否与人工描述表达同一个完成动作。",
      "metrics.li3": "E2E F1（端到端整流程得分）：模型同时预测边界和描述。预测片段必须先通过时间匹配，其描述再通过语义匹配，才算一个正确结果。",
      ...
      "metrics.label.example": "固定分段标注得分：直接采用 G0、G1、G2 的人工边界，模型只为每段生成描述。评测沿用 <a href=\"https://macrodata.co/blog/annotating-robot-video-subtasks\" target=\"_blank\" rel=\"noopener\">Macrodata 公开的 LLM-as-judge rubric</a>：将人工描述、模型描述和 episode instruction 交给 Gemini-3.5-Flash，逐段返回 <code>match=true/false</code>。若结果为 <code>[true, true, false]</code>，则正确描述数为 2，Label Acc（固定分段标注得分）= 正确描述数 / 人工分段数 = 2/3≈66.7%。实际评判 prompt 见 <a href=\"#app-prompts\">附录 F</a>。",
      "metrics.e2e.example": "端到端整流程得分：保留模型预测的 4 个片段及其描述。时间匹配先得到 P0–G0 与 P1–G1；若语义评判只有 P0–G0 返回 <code>true</code>，则最终正确结果数 s=1。预测命中率（precision）= s / 预测分段数 = 1/4，参考找回率（recall）= s / 人工分段数 = 1/3，因此 E2E F1（端到端整流程得分）= 2s / (预测分段数 + 人工分段数) = 2/(4+3)≈0.286。",
      ...
      "metrics.toy.3": "一对一时间匹配共 2 个。4 个预测片段中有 2 个匹配，预测命中率（precision）为 2/4=0.50；3 个人工片段中有 2 个被找回，参考找回率（recall）为 2/3≈0.67；Segment F1（综合分段得分）≈0.571。",
    ...
    en: {
      ...
      "metrics.lead": "The three scores evaluate progressively broader capabilities: temporal boundary accuracy, description quality when boundaries are given, and the overall end-to-end performance of the segmentation-and-labeling pipeline. See <a href=\"#app-metrics\">Appendix A</a> for the formulas.",
      ...
      "metrics.li2": "Fixed-boundary Label Acc uses the human boundaries directly, so the model predicts no timestamps. It measures only whether each generated description expresses the same completed action as the human description.",
      "metrics.li3": "Semantic E2E F1 evaluates predicted boundaries and descriptions together. A predicted segment must first pass temporal matching and then pass semantic matching to count as correct.",
      ...
      "metrics.label.example": "Fixed-boundary Label Acc: use the human boundaries G0, G1, and G2 directly; the model only generates one description per segment. Following <a href=\"https://macrodata.co/blog/annotating-robot-video-subtasks\" target=\"_blank\" rel=\"noopener\">Macrodata's published LLM-as-judge rubric</a>, Gemini-3.5-Flash receives the human description, predicted description, and episode instruction, then returns one JSON match decision per segment, such as <code>{\"match\": true}</code>. If the three results are <code>[true, true, false]</code>, two descriptions are correct, so Label Acc (fixed-boundary labeling score) = correct descriptions / human segments = 2/3≈66.7%. See <a href=\"#app-prompts\">Appendix F</a> for the actual judge prompt used in this report.",
      "metrics.e2e.example": "Semantic E2E F1 (end-to-end score): retain the four predicted spans and their descriptions. Temporal matching first yields P0–G0 and P1–G1. If semantic judging returns <code>true</code> only for P0–G0, the final true-positive count is s=1. Precision = s / predicted segments = 1/4, recall = s / human segments = 1/3, and therefore E2E F1 = 2s / (predicted segments + human segments) = 2/(4+3)≈0.286.",
