...
(function () {
  const I18N = {
    zh: {
      ...
      "world.p.rule": "<a href=\"https://microsoft.github.io/VITRA/\" target=\"_blank\" rel=\"noopener\">VITRA</a><sup class=\"cite\"><a class=\"cite-ref\" href=\"#ref-vitra\">5</a></sup> 将单目第一视角视频转换为三维手部与相机运动、原子动作片段和语言指令，并以腕部速度的局部极小值作为候选动作边界。另一类方法直接使用预设时长。在 Toyota Research Institute 的 <a href=\"https://co-training-lbm.github.io/\" target=\"_blank\" rel=\"noopener\"><em>A Systematic Study of Data Modalities and Strategies for Co-training Large Behavior Models for Robot Manipulation</em></a><sup class=\"cite\"><a class=\"cite-ref\" href=\"#ref-cotraining\">10</a></sup> 中，研究者构建人类视频语言标注数据时，沿用 <a href=\"https://egovid.github.io/\" target=\"_blank\" rel=\"noopener\">EgoVid</a> 的划分方式，将筛选后的 Ego4D 视频切成固定 4 秒片段，再按 1 秒间隔抽帧生成描述。两类方法都不需要学习专门的边界预测器，但边界未必对应动作真正完成的时刻。基于运动低谷的方案尤其容易<strong>过分割</strong>：动作内部的停顿、重新抓握和手腕微调都可能产生候选边界，使「把胡萝卜放进碗里」这样的简单原子动作也被切成五六段。本文也探索了这一路线，以 HaWoR 腕速低谷生成边界并合并相邻片段；在 HomER 上，该方法预测 810 段，而人工参考为 470 段，Segment F1 为 <strong>0.0953</strong>，印证了过分割问题。为使边界更多地对应语义完整的动作，最终管线改用带时间戳的 contact sheet 进行分段。",
      "world.h3.md": "Macrodata Labs / WGO-Bench：完整流程与公开评测",
      "world.p.md": "Macrodata Labs 的 <a href=\"https://macrodata.co/blog/annotating-robot-video-subtasks\" target=\"_blank\" rel=\"noopener\"><em>Segmenting Robot Video into Actionable Subtasks</em></a><sup class=\"cite\"><a class=\"cite-ref\" href=\"#ref-macrodata\">2</a></sup> 是目前最系统的公开工作：它将「长视频 → 动作级片段 + 短语义描述」定义为可量化评测的任务，发布 WGO-Bench 及分段、固定边界标注和端到端三类指标，给出 contact sheet 与 GEPA 搜索分段规则等设计，并比较多个闭源和开源模型。与本文直接相关的不足是，各项设计的主要收益多在 Gemini 等强闭源模型上验证，尚未系统回答<strong>仅使用开放权重模型时，哪些设计仍然有效</strong>。",
      "world.h3.scale": "Scale Labs：已切好的 clip 上的稠密描述",
      "world.p.scale": "Scale Labs 的 <a href=\"https://labs.scale.com/blog/path-to-large-scale-dense-video-captioning\" target=\"_blank\" rel=\"noopener\">The Path to Large Scale Dense Video Captioning</a><sup class=\"cite\"><a class=\"cite-ref\" href=\"#ref-scale\">4</a></sup> 讨论在<strong>已经切分好的 clip</strong> 上生成稠密描述。其核心工程贡献之一是用 <strong>contact sheet 辅助标注</strong>：将多个稳定抽样帧拼成一张网格图，并结合时间信息提供紧凑的片段表示，从而减少逐帧输入的开销。该设计直接启发了本文的视觉输入形式。不过，该工作默认时间边界已经给定，因此没有涉及从完整长视频中定位动作起止这一更前置的分段问题。",
      "world.h3.egoant": "EgoANT",
      "world.p.egoant": "EgoANT 承接了上述工作的有效设计，同时针对尚未覆盖的问题展开：采用 Scale Labs 的 contact sheet 作为紧凑视觉表示，复用 Macrodata Labs 公开的 completed-event 分段规则，并使用 WGO-Bench 作为实验协议。在此基础上，本文构建仅依赖开放权重 Qwen 模型的完整「长视频 → 动作边界 + 操作描述」流程。与固定时长或腕速规则相比，EgoANT 让模型依据语义完整的动作预测边界；与只处理预切 clip 的方案相比，它从原始长视频开始；相较 Macrodata Labs 以强闭源模型为主的实验，本文重点检验这些设计在开放权重模型下是否仍然有效。后续章节将依次介绍输入形式、分段方法和标注路径。",
    ...
    en: {
      ...
      "world.p.rule": "<a href=\"https://microsoft.github.io/VITRA/\" target=\"_blank\" rel=\"noopener\">VITRA</a><sup class=\"cite\"><a class=\"cite-ref\" href=\"#ref-vitra\">5</a></sup> converts monocular egocentric video into 3D hand and camera motion, atomic action segments, and language instructions, using local minima in wrist speed as candidate action boundaries. A second approach uses predetermined durations. In Toyota Research Institute's <a href=\"https://co-training-lbm.github.io/\" target=\"_blank\" rel=\"noopener\"><em>A Systematic Study of Data Modalities and Strategies for Co-training Large Behavior Models for Robot Manipulation</em></a><sup class=\"cite\"><a class=\"cite-ref\" href=\"#ref-cotraining\">10</a></sup>, the human-video language-annotation data follows <a href=\"https://egovid.github.io/\" target=\"_blank\" rel=\"noopener\">EgoVid</a>'s splits: filtered Ego4D videos are divided into fixed four-second clips and sampled at one-second intervals for description generation. Neither approach requires a learned boundary predictor, but their boundaries need not coincide with the completion of a semantic action. Motion-minimum methods are especially prone to <strong>over-segmentation</strong>: pauses, regrasps, and small wrist adjustments within one action can all introduce candidate boundaries, causing even a simple atomic action such as \"put the carrot in the bowl\" to be split into five or six pieces. We also explored this approach using HaWoR wrist-speed minima followed by adjacent-segment merging. On HomER it predicts 810 segments against 470 human references and reaches Segment F1 <strong>0.0953</strong>, confirming the over-segmentation problem. The final pipeline instead uses timestamped contact sheets so that boundaries are guided by semantically complete actions.",
      "world.h3.md": "Macrodata Labs / WGO-Bench: a complete pipeline and public evaluation",
      "world.p.md": "Macrodata Labs' <a href=\"https://macrodata.co/blog/annotating-robot-video-subtasks\" target=\"_blank\" rel=\"noopener\"><em>Segmenting Robot Video into Actionable Subtasks</em></a><sup class=\"cite\"><a class=\"cite-ref\" href=\"#ref-macrodata\">2</a></sup> is the most systematic public treatment so far. It defines \"long video &rarr; action-level segments + short semantic descriptions\" as a measurable task, releases WGO-Bench with segmentation, fixed-boundary labeling, and end-to-end metrics, introduces designs including contact sheets and GEPA-searched segmentation rules, and compares several closed and open models. The gap most relevant here is that the reported gains are validated primarily on strong closed models such as Gemini, leaving open <strong>which design choices remain effective with open-weight models alone</strong>.",
      "world.h3.scale": "Scale Labs: dense captioning on pre-cut clips",
      "world.p.scale": "Scale Labs' <a href=\"https://labs.scale.com/blog/path-to-large-scale-dense-video-captioning\" target=\"_blank\" rel=\"noopener\">The Path to Large Scale Dense Video Captioning</a><sup class=\"cite\"><a class=\"cite-ref\" href=\"#ref-scale\">4</a></sup> studies dense descriptions over <strong>already-cut clips</strong>. One of its main engineering contributions is contact-sheet-assisted annotation: stable sampled frames are tiled into a single grid and paired with temporal information to provide a compact visual representation, reducing the cost of sending frames individually. This design directly informed our visual-input format. However, the Scale setting assumes temporal boundaries are already given and therefore does not address the preceding problem of locating action boundaries in a complete long video.",
      "world.h3.egoant": "EgoANT",
      "world.p.egoant": "EgoANT carries forward the effective elements of these prior systems while addressing what they leave open. It uses Scale Labs' contact sheets as a compact visual representation, reuses Macrodata Labs' published completed-event segmentation rules, and adopts WGO-Bench as the experimental protocol. Building on those components, this report develops a complete \"long video &rarr; action boundaries + operation descriptions\" workflow using open-weight Qwen models only. Unlike fixed-duration or wrist-speed rules, EgoANT predicts boundaries from semantically complete actions; unlike methods limited to pre-cut clips, it starts from raw long video; and unlike Macrodata Labs' experiments centered on strong closed models, it tests which design choices remain effective in an open-weight stack. The following sections introduce the input representation, segmentation method, and labeling path in turn.",
