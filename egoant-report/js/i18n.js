...
(function () {
  const I18N = {
    zh: {
      ...
      "nav.world": "相关工作",
      "nav.terms": "术语约定",
      "pipeline.h3.cost": "4.5 成本对照",
      "pipeline.h3.recipe": "4.4 推荐配置",
      "pipeline.e2e.lead": "端到端要解决的问题是：模型同时给出预测边界与每段描述，评测先按时间 IoU 配对，再判断语义是否匹配。除腕速一体基线外，本组固定 S2 最优分段边界（视频分段 F1 0.2031，预测 308 段 / 参考 470 段），只改变描述生成路径。分数统一为端到端整流程得分（E2E F1）。",
      "pipeline.h3.e2e": "4.3 端到端",
      "pipeline.label.lead": "固定边界标注要解决的问题是：给定人工参考时间边界，为每个片段生成简短操作描述。本阶段不预测时间，只评描述是否与人工描述表达同一完成动作。分数统一为同一 HomER 子集上的固定分段标注得分（Label Acc）。",
      "pipeline.h3.label": "4.2 固定边界标注",
      "pipeline.seg.analysis": "<p>结合上图与表，按实验路径可以归纳如下。</p><ol><li>腕速规则切段并合并：预测片段 810 段，远多于参考分段 470 段，配对成功仅 61。腕速低谷会把动作内部的停顿和微调也当成边界，因此整体呈过分割，难以对齐语义完整的动作。</li><li>拼贴图分片（每次最多 3 张）：改用带时间戳拼贴图后，模型可以在局部时间上下文中判边界，但仍受请求长度限制。每次最多送入 3 张拼贴图，请求之间的接缝缺乏整集上下文，容易被误判为动作边界。</li><li>整集一次提交（无切段规则清单）：整集同一次调用消除了分片接缝带来的伪边界，但模型偏保守：预测片段仅 148 段（参考 470 段），配对成功 38，多数参考动作未被单独切出，整体表现为欠分割。</li><li>整集一次提交 + 切段规则清单：加入切段规则后，预测片段升至 202 段、配对成功 46，相对无规则清单有提升，但仍远少于参考 470 段。规则清单改善了「该切哪些完成事件」，却不足以单独决定切分粒度。</li><li>S1 加密切分：通过提示词要求更密的候选切分后，配对成功升至 80（参考分段共 470），预测片段增至 558 段。召回改善的同时过分割回升，说明密切可以抬高找回率，但边界仍需第二遍精修。</li><li>S2 局部精修：在粗边界附近开窗重切，是本组主要增益来源。窗口不外扩优于向外多看 0.5/1/2 秒；在窗口不外扩时，写入覆盖完整动作提示词优于无覆盖完整动作提示词，也优于用算法中点事后补覆盖。最终「窗口不外扩 + 覆盖完整动作提示词」取得最高视频分段 F1 0.2031（配对成功 79，预测 308 段，参考 470 段）。</li><li>S2 + 相邻片段规则合并：三种规则都接在上述最优 S2 预测之后，只根据相邻预测段的标签与时间间隙做后处理。结果均低于未合并的 S2，说明在当前 IoU 评测口径下，粗合并并不能替代局部精修；细节见 <a href=\"#app-rule-merge\">附录 C.2</a>。</li><li>模型对比：在相同的拼贴图分片设置下，Qwen3.6-27B 的视频分段 F1 为 0.1278，高于 Qwen3.5-397B 的 0.0952；最终最优 S2 配置也采用 27B。</li></ol>",
      ...
      "intro.p3": "EgoANT 是面向第一视角人类操作视频的自动标注流程：先将长视频划分为动作级片段，再为每个片段生成简洁的操作描述。在 HomER 的 25 个视频、470 个参考片段上，最佳分段配置的视频分段 F1 为 <strong>0.2031</strong>；在固定参考边界上，语义标注的 Label Acc 为 <strong>55.7%</strong>；仅以视频为输入的最佳端到端配置取得 Semantic E2E F1 <strong>0.1542</strong>。这些结果表明，完全基于开放权重视觉语言模型构建可复现的第一视角子任务标注流程是可行的。",
      "tldr.h2": "TL;DR",
      ...
      "tldr.pt1": "<strong>先选对模型，再考虑复杂的视觉输入增强。</strong>给定人工分段、只评估片段描述时，Qwen3.6-27B 使用原始帧取得 55.7% 的固定分段标注得分；改用多帧拼图或叠加启发式视觉提示后，得分分别降至 52.8% 和 50.6%。在输入和提示词完全相同的分段实验中，Qwen3.6-27B 的视频分段 F1 为 0.1278，高于 Qwen3.5-397B-A17B 的 0.0952。就本组实验而言，模型选择带来的收益大于额外的视觉输入技巧。",
      "tldr.pt2": "<strong>最佳分段方案是「整集粗分 + 局部精修」。</strong>系统先用时间戳网格浏览完整视频并给出粗边界，再在每个粗边界附近重新判断动作起止。相较腕部速度规则基线，这一方案将视频分段 F1 从 0.0953 提升至 <strong>0.2031</strong>，并在 470 个人工参考片段中找到 79 个合格的时间匹配。Macrodata 公布的同范围 Gemini 视频分段 F1 为 0.227，与本方案相差 0.024。",
      ...
      "cost.note": "两种 WGO 路径共用同一套 S2 分段（视频分段 F1 = 0.2031），仅标注调用不同。API 次数来自报告产物计数，输入 token 根据图片数量与分辨率估算，输出 token 则按任务类型设置区间。Qwen3.6-27B 按输入 $0.422/M、输出 $2.532/M 计价；Qwen3.5-397B-A17B 按输入 $0.1644/M、输出 $0.9864/M 计价。流程未使用网页搜索，因此不计 $0.000548/次的搜索费。美元结果不含离线 Gemini judge，也不能与 Macrodata 的 Gemini batch 账单视为同条件价格比较。细节见附录 F。",
      ...
      "world.p.rule": "<a href=\"https://microsoft.github.io/VITRA/\" target=\"_blank\" rel=\"noopener\">VITRA</a><sup class=\"cite\"><a class=\"cite-ref\" href=\"#ref-vitra\">5</a></sup> 将单目第一视角视频转换为三维手部与相机运动、原子动作片段和语言指令，并以腕部速度的局部极小值作为候选动作边界。另一类方法直接使用预设时长。在 Toyota Research Institute 的 <a href=\"https://co-training-lbm.github.io/\" target=\"_blank\" rel=\"noopener\"><em>A Systematic Study of Data Modalities and Strategies for Co-training Large Behavior Models for Robot Manipulation</em></a><sup class=\"cite\"><a class=\"cite-ref\" href=\"#ref-cotraining\">10</a></sup> 中，研究者构建人类视频语言标注数据时，沿用 <a href=\"https://egovid.github.io/\" target=\"_blank\" rel=\"noopener\">EgoVid</a> 的划分方式，将筛选后的 Ego4D 视频切成固定 4 秒片段，再按 1 秒间隔抽帧生成描述。两类方法都容易<strong>过分割</strong>：运动低谷会把动作内部的停顿、重新抓握和手腕微调误判为边界；固定时长切分则不考虑动作何时真正完成，可能在同一动作内部反复插入切点，使「把胡萝卜放进碗里」这样的简单原子动作也被拆成多个片段。本文探索了基于 HaWoR 腕速低谷的分段方法，并合并相邻片段；在 HomER 上，该方法预测 810 段，而人工参考为 470 段，视频分段 F1 为 0.0953，印证了过分割问题。一种应对方式是将连续抽帧拼成带时间戳的拼贴图，让模型从更完整的时间上下文中判断动作边界，从而减少不必要的切分。",
      ...
      "fig.src.metric": "assets/explain/metric_iou_f1_zh.svg?v=20260811-17",
      ...
      "world.p.inputs": "后文实验用到六类视觉输入：原始帧、时序拼贴、拼贴图、邻段拼贴、视觉提示叠加、手部裁剪。下图以 HomER 数据集中的一个样本片段（22.9–26.8 秒，「拉开床头柜抽屉」）展示各自的画面组织形式。",
      ...
      "world.zoo.crop": "<strong>手部裁剪</strong>：左图橙色框表示裁剪区域，右图是实际送入标注模型的局部图像。它与在完整画面上保留轨迹叠加是两种不同输入。",
      ...
      "eval.h3.metrics": "三种分数分别测试什么能力",
      "metrics.lead": "三种分数依次衡量：时间边界是否准确、给定边界后的描述是否正确，以及端到端分段与标注的整体表现。下面用同一个 10 秒例子给出计算步骤与公式。",
      "metrics.li1": "视频分段 F1（Segment F1）：只评时间边界。对一对预测片段与参考片段，时间 IoU 等于「两段重叠的时长 ÷ 两段合起来覆盖的总时长」；完全重合时为 1，完全不重合时为 0。IoU≥0.75 的片段对才有资格形成一对一时间匹配。",
      ...
      "metrics.h3.label": "标注得分例子",
      "metrics.h3.e2e": "端到端得分例子",
      "metrics.label.example": "沿用同一 10 秒例子的人工边界 G<sub>0</sub>、G<sub>1</sub>、G<sub>2</sub>，模型只为每段生成描述。评测沿用 <a href=\"https://macrodata.co/blog/annotating-robot-video-subtasks\" target=\"_blank\" rel=\"noopener\">Macrodata 公开的 LLM-as-judge rubric</a>：将人工描述、模型描述和 episode instruction 交给 Gemini-3.5-Flash，逐段返回 <code>match=true/false</code>。若结果为 <code>[true, true, false]</code>，则正确描述数 c = 2。实际评判 prompt 见 <a href=\"#app-prompts\">附录 E</a>。",
      ...
      "appendix.h2": "6. 附录：概念、公式、实现与成本记账",
      "tldr.k.seg": "视频分段 F1（Segment F1）",
      ...
      "story.seg.1": "腕速规则切段并合并：用 HaWoR 从视频重建手部运动并估计腕部速度曲线，在速度局部低谷处放置候选切点，再对相邻片段做规则合并。",
      ...
      "story.seg.4": "整集一次提交 + 切段规则清单：整集拼贴图一次提交，提示词改用 Macrodata 用 GEPA 搜索得到的切段规则（见 <a href=\"#app-prompts\">附录 E</a> · <a href=\"prompts/gepa_completed_events_duration_prior_v1.md\" download>下载</a>）。",
      "story.seg.5": "S1 加密切分：对整集做一次分段，通过修改提示词要求模型输出更多、更短的候选片段。",
      "story.seg.6": "S2 局部精修：先由整集粗分得到粗边界，再围绕每个粗边界开局部时间窗，把窗内拼贴图交回模型重切。「覆盖完整动作提示词」要求窗口内看得见起止的完成操作都要切出（见术语约定）。得分表中的变体包括：窗外扩约 1 秒、窗口不外扩（无覆盖完整动作提示词）、窗外扩 0.5/1/2 秒；另在「无覆盖完整动作提示词」的预测上，用脚本按中点规则事后补边界，记为「算法补覆盖」；最优设置为窗口不外扩 + 覆盖完整动作提示词（精修提示词见 <a href=\"#app-prompts\">附录 E</a> · <a href=\"prompts/s2_fullcover_refine.md\" download>下载</a>）。",
      "story.seg.7": "S2 + 相邻片段规则合并：输入是最优 S2 产生的预测片段列表，输出是合并后的预测片段列表。脚本按时间顺序只比较相邻段，三种规则分别是：标签规范化后完全相同则合并；主要动词/物体相同则合并；先桥接很短的时间空隙，再按前两类规则尝试合并。伪代码见 <a href=\"#app-rule-merge\">附录 C.2</a>。",
      ...
      "story.label.cap1": "读图（三栏）：左为原始帧；中为启发式框（画面中心偏下固定方框，不是手腕检测）；右为送给标注模型的裁剪图。该近似裁剪路径在固定边界上低于原始帧。详见 <a href=\"#app-visual\">附录 D</a>。",
      ...
      "appendix.lead": "正文讲「试了什么、分数怎么变」；本附录补清楚术语和实现边界。目录： <a href=\"#app-seg\">A 分段概念</a> · <a href=\"#app-e2e\">B 标注/E2E 术语</a> · <a href=\"#app-prod\">C 腕速基线</a> · <a href=\"#app-rule-merge\">C.2 相邻片段合并</a> · <a href=\"#app-visual\">D 视觉输入</a> · <a href=\"#app-prompts\">E Prompt</a> · <a href=\"#app-cost\">F 成本记账</a> · <a href=\"#audit\">G 效度</a>。",
      ...
      "metrics.toy.1": "吸附后，预测片段变为 P<sub>0</sub>[0,2.8]、P<sub>1</sub>[2.8,5.5]、P<sub>2</sub>[5.5,8]、P<sub>3</sub>[8,10]。只有 P<sub>0</sub> 和 P<sub>3</sub> 的外侧端点发生变化。",
      "metrics.toy.2": "P<sub>0</sub>–G<sub>0</sub>、P<sub>1</sub>–G<sub>1</sub> 的时间 IoU 达到 0.75 阈值，形成一对一配对；P<sub>2</sub>、P<sub>3</sub> 未达到阈值。",
      "metrics.toy.3": "一对一时间匹配数 m = 2，预测段数 n<sub>pred</sub> = 4，参考段数 n<sub>gold</sub> = 3。",
      "metrics.seg.formula": "IoU = 重叠时长 / 两段合起来覆盖的总时长\n本例：P<sub>0</sub>–G<sub>0</sub> = 2.8 / 3 ≈ 0.933；P<sub>1</sub>–G<sub>1</sub> = 2.5 / 3.2 ≈ 0.781（均 ≥ 0.75）\n预测命中率 = m / n<sub>pred</sub> = 2 / 4 = 0.50\n参考找回率 = m / n<sub>gold</sub> = 2 / 3 ≈ 0.67\n视频分段 F1 = 2m / (n<sub>pred</sub> + n<sub>gold</sub>) = 2×2 / (4 + 3) = 4/7 ≈ 0.571",
    ...
    en: {
      ...
      "pipeline.seg.lead": "Video segmentation asks: given one long egocentric video, output action-level time boundaries. The input is the full video; the output is a set of start–end intervals (for example [12.0, 18.5]). This stage scores boundary alignment only. All scores are Segment F1 on the same HomER subset. Below we list the segmentation methods we tried and their segmentation scores.",
      ...
      "pipeline.h2": "4. Annotation pipeline",
      ...
      "tldr.pt2": "<strong>The best segmentation recipe is a coarse whole-episode pass followed by local refinement.</strong> The model first scans timestamped grids for the complete video, proposes coarse boundaries, and then re-segments short windows around those boundaries. This raises Segment F1 from 0.0953 for the wrist-speed rule baseline to <strong>0.2031</strong>, yielding 79 valid temporal matches among 470 human reference segments. Macrodata reports a Gemini Segment F1 of 0.227 on the same subset, 0.024 above this result.",
      ...
      "cost.note": "Both WGO paths share the same S2 segmentation (Segment F1 = 0.2031) and differ only in labeling calls. API counts come from report artifacts; input tokens are estimated from image counts and resolution, while output tokens use task-specific ranges. Pricing is $0.422/M input and $2.532/M output for Qwen3.6-27B, and $0.1644/M input and $0.9864/M output for Qwen3.5-397B-A17B. The pipeline does not use web search, so the $0.000548/request search fee is excluded. Dollar estimates exclude the offline Gemini judge and are not directly comparable to Macrodata's Gemini batch invoice. See Appendix F.",
      ...
      "fig.src.metric": "assets/explain/metric_iou_f1.svg?v=20260811-17",
      ...
      "world.p.inputs": "Later experiments use six visual-input formats: raw frames, a temporal collage, a contact sheet, a neighbor collage, a visual-hint overlay, and a hand crop. The examples below use one sample clip from the HomER dataset (22.9&ndash;26.8s, \"open the nightstand drawer\") to show how each format organizes the frames.",
      ...
      "world.zoo.crop": "<strong>Hand crop</strong>: the orange box on the left marks the crop region, and the right panel shows the local image actually sent to the labeler. This is distinct from keeping trajectory overlays on the full frame.",
      ...
      "metrics.li1": "Segment F1 evaluates temporal boundaries only. For a predicted/reference pair, temporal IoU is the duration of their overlap divided by the total duration covered by either segment. It is 1 for identical spans and 0 for disjoint spans. Only pairs with IoU≥0.75 are eligible for one-to-one temporal matching.",
      ...
      "walk.h2": "5. Case study: end-to-end flow on a HomER sample",
      "walk.lead": "The case below follows the recommended path from Section 4: predict segment boundaries first, then generate multiple candidate descriptions on those predicted boundaries and let the selector finalize the label. Task: wipe a table / cabinet surface with a cloth. Folded panels keep the original English prompts.",
      ...
      "appendix.h2": "6. Appendix: concepts, formulas, implementation, and cost accounting",
      "tldr.k.seg": "Segment F1",
      ...
      "walk.prompt.sel": "Prompt (English only) — selector",
      "walk.prompt.judge": "Prompt (English only) — judge (scoring only)",
      "story.seg.1": "Wrist-speed rule cuts + merge: reconstruct hand motion with HaWoR, estimate the wrist-speed curve, place candidate cuts at local speed minima, then merge adjacent segments with rules.",
      ...
      "story.seg.4": "Whole-episode request + segmentation-rule list: submit every contact sheet from the episode in one call, using the GEPA-searched rule text from Macrodata (see <a href=\"#app-prompts\">Appendix E</a> · <a href=\"prompts/gepa_completed_events_duration_prior_v1.md\" download>download</a>).",
      ...
      "story.label.cap1": "How to read (3 panels): left = raw frame; middle = heuristic box (fixed lower-center square, not wrist detection); right = crop fed to the labeler. This approximate-crop path scores below raw frames on fixed bounds. See <a href=\"#app-visual\">Appendix D</a>.",
      ...
      "appendix.lead": "The body covers what we tried and how scores moved; this appendix spells out terminology and implementation details. Contents: <a href=\"#app-seg\">A segmentation concepts</a> · <a href=\"#app-e2e\">B label/E2E terms</a> · <a href=\"#app-prod\">C production</a> · <a href=\"#app-rule-merge\">C.2 adjacent merges</a> · <a href=\"#app-visual\">D visuals</a> · <a href=\"#app-prompts\">E prompts</a> · <a href=\"#app-cost\">F cost</a> · <a href=\"#audit\">G validity</a>.",
      ...
      "metrics.toy.1": "After outer snap, the predictions are P<sub>0</sub>[0,2.8], P<sub>1</sub>[2.8,5.5], P<sub>2</sub>[5.5,8], and P<sub>3</sub>[8,10]. Only P<sub>0</sub>'s start and P<sub>3</sub>'s end change.",
      "metrics.toy.2": "P<sub>0</sub>–G<sub>0</sub> and P<sub>1</sub>–G<sub>1</sub> clear the 0.75 temporal-IoU threshold and form one-to-one matches; P<sub>2</sub> and P<sub>3</sub> do not.",
      "metrics.toy.3": "One-to-one temporal matches m = 2, predicted segments n<sub>pred</sub> = 4, and reference segments n<sub>gold</sub> = 3.",
      "metrics.seg.formula": "IoU = overlap duration / total duration covered by either span\nThis example: P<sub>0</sub>–G<sub>0</sub> = 2.8 / 3 ≈ 0.933; P<sub>1</sub>–G<sub>1</sub> = 2.5 / 3.2 ≈ 0.781 (both ≥ 0.75)\nPrecision = m / n<sub>pred</sub> = 2 / 4 = 0.50\nRecall = m / n<sub>gold</sub> = 2 / 3 ≈ 0.67\nSegment F1 = 2m / (n<sub>pred</sub> + n<sub>gold</sub>) = 2×2 / (4 + 3) = 4/7 ≈ 0.571",
      "metrics.label.formula": "Fixed-boundary Label Acc = c / n<sub>gold</sub> = 2 / 3 ≈ 66.7%",
      "metrics.e2e.formula": "End-to-end F1 = 2s / (n<sub>pred</sub> + n<sub>gold</sub>) = 2×1 / (4 + 3) = 2/7 ≈ 0.286",
      "story.chart.label": "Fixed-boundary Label Acc",
  ...
  const APPENDIX_HTML = {
    zh: `
      ...
      <p class="plain">正文讲“试了什么、分数怎么变”；附录补清楚术语和实现边界。目录：
        <a href="#app-seg">A 分段概念</a> ·
        <a href="#app-e2e">B 标注/E2E 术语</a> · <a href="#app-prod">C 腕速基线</a> ·
        <a href="#app-rule-merge">C.2 相邻片段合并</a> ·
        <a href="#app-visual">D 视觉输入</a> · <a href="#app-prompts">E Prompt</a> ·
        <a href="#app-cost">F 成本</a> · <a href="#audit">G 效度</a>。</p>

      

      <h3 id="app-seg">A. 分段概念卡</h3>
      <figure class="figure">
      ...
      <article class="concept-card">
        <h4>第二遍局部精修（S2）</h4>
        <p>粗分之后，在每条粗边界附近开局部时间窗，再用同样的 timestamped contact sheet 细切一次。最终版本使用 <strong>pad=0 + full-cover prompt</strong>，视频分段 F1 到 0.2031。</p>
      </article>
      <article class="concept-card">
        <h4>窗口不外扩（pad=0）</h4>
      ...
      <h3 id="app-prod">C. EgoANT 的腕速基线管线（并澄清与 VITRA 的关系）</h3>
      <div class="pipeline">
      ...
      <p>VITRA 启发的是“先手部/运动信号，再 caption”的问题设定；本系统在这一版里用 HaWoR 腕速作为切段信号，不把 VITRA 当作后端模型。它的主要失败模式是<strong>过分割</strong>：动作中途的犹豫或微调在速度曲线上也像边界，后续 merge judge 虽可合并一部分，但在 WGO 的 IoU 口径下仍会拉低视频分段 F1。</p>

      <h3 id="app-rule-merge">C.2 相邻片段规则合并伪代码</h3>
      <p>这组实验不是新的模型调用，也不是 production merge judge。它只对最优 S2 分段输出做离线后处理，用来验证“把相邻预测段合并得更像完整动作”是否能提高视频分段 F1。</p>
...
def compatible(a, b, strategy):
    ...
    if strategy == "bridge_short_gap":
      ...
      <p>实现边界：脚本只读取预测 JSON 的 <code>start_sec</code>、<code>end_sec</code>、<code>subtask</code>，按相邻段规则合并；不能新增边界，只能删除或移动已有预测段之间的边界。HomER 上三种规则都降低得分，说明 S2 的细边界已经比这些粗合并更适合 WGO 的 IoU 口径。</p>

      <h3 id="app-visual">D. 视觉输入对照</h3>
      <figure class="figure">
        <img src="assets/explain/visual_input_taxonomy_zh.svg" alt="Visual input taxonomy" />
        <figcaption>这些视觉输入的作用不同：拼贴图让模型看完整时间轴，用于找边界；原始帧、视觉提示叠加、手部裁剪、时序/邻段拼贴则是在边界已固定时，比较哪种视觉证据更利于写对当前动作。</figcaption>
      </figure>
      <table><thead><tr><th>名称</th><th>模型看见什么</th><th>典型用途</th><th>HomER 上</th></tr></thead><tbody>
        <tr><td>拼贴图</td><td>带时间戳的抽帧网格</td><td>分段</td><td>主路径</td></tr>
        <tr><td>原始帧</td><td>段内均匀原始帧</td><td>标注默认</td><td>固定分段标注得分 55.7%（27B）/ 50.2%（397B）</td></tr>
        <tr><td>视觉提示叠加</td><td>原始帧 + 光流或启发式运动提示</td><td>标注消融</td><td>50.6%（27B）/ 48.5%（397B）</td></tr>
        <tr><td>时序拼贴</td><td>过去 / 当前 / 未来整帧网格</td><td>标注消融</td><td>52.8%（27B）/ 45.1%（397B）</td></tr>
        <tr><td>邻段拼贴</td><td>上一段 / 当前段 / 下一段抽样帧</td><td>标注消融</td><td>39.6–40.0%（397B）</td></tr>
        <tr><td>HaWoR 手部裁剪</td><td>按 HaWoR 估计腕轨裁剪手部区域</td><td>标注候选</td><td>50.9%（397B）</td></tr>
      </tbody></table>

      <h3 id="app-prompts">E. Prompt 原文（English）</h3>
      ...
      <h3 id="app-cost">F. 成本：估计与公开数字对照</h3>
    ...
    en: `
      ...
      <p class="plain">The body explains what we tried and how scores moved. This appendix spells out terminology and implementation boundaries. Contents:
        <a href="#app-seg">A segmentation concepts</a> ·
        <a href="#app-e2e">B label/E2E terms</a> · <a href="#app-prod">C production</a> ·
        <a href="#app-rule-merge">C.2 adjacent merges</a> ·
        <a href="#app-visual">D visuals</a> · <a href="#app-prompts">E prompts</a> ·
        <a href="#app-cost">F cost</a> · <a href="#audit">G validity</a>.</p>

      

      <h3 id="app-seg">A. Segmentation concept cards</h3>
      <figure class="figure">
      ...
      <h3 id="app-e2e">B. Labeling / E2E terms</h3>
      ...
      <h3 id="app-prod">C. The EgoANT wrist-speed baseline (and how it relates to VITRA)</h3>
      <div class="pipeline">
      ...
      <h3 id="app-rule-merge">C.2 Adjacent rule-merge pseudocode</h3>
...
def compatible(a, b, strategy):
    ...
    if strategy == "bridge_short_gap":
      ...
      <h3 id="app-prompts">E. Prompt originals (English)</h3>
      ...
      <h3 id="app-cost">F. Cost: estimates and published numbers</h3>
      ...
      <h3 id="audit">G. Validity notes</h3>
