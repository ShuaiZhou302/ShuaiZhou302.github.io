...
(function () {
  const I18N = {
    zh: {
      ...
      "pipeline.seg.analysis": "<p>结合上图与表，对各方法简要评价如下：</p><ol><li>腕速规则切段并合并：预测片段远多于人工参考（810 vs 470），视频分段 F1 仅 0.0953，说明运动低谷信号难以对齐动作语义边界。</li><li>拼贴图分片：请求接缝容易被当成动作边界。同设置下 397B 为 0.0952、27B 为 0.1278；换更大模型并不能消解分片带来的伪边界。</li><li>整集一次提交（无切段规则清单）：去掉分片接缝后伪边界减少，但预测片段偏少（148），F1 为 0.1230。</li><li>整集一次提交 + 切段规则清单：F1 升至 0.1369，比无规则清单更好，整体仍偏保守。</li><li>S1 加密切分：召回上升（match 80），F1 到 0.1556，同时预测段数增至 558，切分更细。</li><li>S2 局部精修：是主要增益来源。早期窗外扩约 1 秒版为 0.1674；窗口不外扩但尚未写「盖住完整动作」时为 0.1711，外扩 0.5/1/2 秒均更低；算法补覆盖后处理为 0.1635，不及把约束写进提示词。最终「窗口不外扩并盖住完整动作」取得最高视频分段 F1 0.2031。</li><li>相邻片段规则合并：三种策略都基于上述最优 S2 预测做后处理，F1 分别降为 0.1987 / 0.1947 / 0.1883，因此不作为默认配置。</li></ol><p>模型对比：同一拼贴图分片设置下 27B 高于 397B；最终最优 S2 配置也落在 27B。说明在本组实验里，分段流程与提示词设计带来的增益，往往大于单纯换更大模型。</p>",
      ...
      "story.seg.1": "腕速规则切段并合并：用 HaWoR 从视频重建手部运动并估计腕部速度曲线，在速度局部低谷处放置候选切点，再对相邻片段做规则合并。这是原 EgoANT 生产管线中的启发式基线，不调用视觉语言模型。",
      "story.seg.2": "拼贴图分片（每次最多 3 张）：把整集拼贴图按请求拆开，每次最多送入 3 张，由模型只在该局部时间范围内预测边界。",
      ...
      "story.seg.6": "S2 局部精修：先由整集粗分得到粗边界，再围绕每个粗边界开局部时间窗，把窗内拼贴图交回模型重切。我们比较了窗口是否向外扩展，以及是否在提示词中要求「盖住完整动作」；另测过用算法中点覆盖补全边界的后处理（精修提示词见 <a href=\"#app-prompts\">附录 F</a> · <a href=\"prompts/s2_fullcover_refine.md\" download>下载</a>）。",
      "story.seg.7": "相邻片段规则合并：在 S2「窗口不外扩并盖住完整动作」的预测结果上，用规则脚本按时间相邻、标签完全相同、动词/物体相同或短间隙桥接等策略合并片段，不再调用模型。该后处理为内部脚本，附录未单独提供下载。",
      "story.chart.seg": "视频分段 F1（Segment F1）",
      "story.seg.modelnote": "条件名中的「· 27B / · 397B」分别表示该次调用使用 Qwen3.6-27B / Qwen3.5-397B-A17B。",
      "story.seg.legend": "表头：P（Precision）= match / pred（预测段里配对成功的比例）；R（Recall）= match / gold（人工参考段被找回的比例）；match / pred / gold = 配对成功数 / 预测段数 / 人工参考段数（本子集人工参考恒为 470）。",
      "th.segf1": "视频分段 F1",
    ...
    en: {
      ...
      "pipeline.seg.analysis": "<p>Reading the chart and table together, a short take on each method:</p><ol><li>Wrist-speed rule cuts + merge: far more predictions than the human reference (810 vs 470) and only 0.0953 video-segmentation F1, so motion minima are a poor match to action semantics.</li><li>Chunked contact sheets: request seams are often treated as action boundaries. Under the same setting, 397B scores 0.0952 and 27B scores 0.1278; a larger model does not remove the chunking artifact.</li><li>Whole-episode request without the rule list: seams disappear, but predictions are too few (148) and F1 is 0.1230.</li><li>Whole-episode request with the rule list: F1 rises to 0.1369, better than the no-rule prompt, still conservative overall.</li><li>S1 denser cuts: recall rises (match 80) and F1 reaches 0.1556, with predictions growing to 558 and finer cuts.</li><li>S2 local refinement is the main gain. The early ≈1s pad-out run scores 0.1674; no pad-out before the cover-full-actions prompt scores 0.1711, while 0.5/1/2s pad-out are all worse; algorithmic cover postprocessing scores 0.1635 and underperforms writing the constraint into the prompt. The final no-pad-out + cover-full-actions setting reaches the best video-segmentation F1 of 0.2031.</li><li>Adjacent-segment rule merges: all three strategies postprocess that best S2 prediction and drop F1 to 0.1987 / 0.1947 / 0.1883, so they are not defaults.</li></ol><p>Model comparison: under identical chunked contact sheets, 27B beats 397B; the best S2 setting also lands on 27B. In this suite, pipeline and prompt design usually matter more than simply using a larger model.</p>",
      ...
      "walk.prompt.sel": "Prompt (English only) — selector",
      "walk.prompt.judge": "Prompt (English only) — judge (scoring only)",
      "story.seg.1": "Wrist-speed rule cuts + merge: reconstruct hand motion with HaWoR, estimate the wrist-speed curve, place candidate cuts at local speed minima, then merge adjacent segments with rules. This is the original EgoANT production heuristic and does not call a vision-language model.",
      "story.seg.2": "Chunked contact sheets (max 3 per call): split the episode’s contact sheets across requests, sending at most three sheets each time, so the model predicts boundaries only inside that local time span.",
      ...
      "story.seg.6": "S2 local refinement: take coarse bounds from the whole-episode pass, open a local time window around each bound, and re-cut from the in-window contact sheets. We compared whether the window expands outward and whether the prompt requires covering completed actions; we also tested algorithmic midpoint-cover postprocessing (refine prompt: <a href=\"#app-prompts\">Appendix F</a> · <a href=\"prompts/s2_fullcover_refine.md\" download>download</a>).",
      "story.seg.7": "Adjacent-segment rule merges: starting from the S2 “no pad-out + cover full actions” predictions, merge with rule scripts based on temporal adjacency, identical labels, shared verb/object, or short-gap bridging, without another model call. These postprocess scripts are internal and are not shipped in the appendix.",
      ...
      "story.seg.legend": "Headers: P (Precision) = match / pred; R (Recall) = match / gold; match / pred / gold = counts (human reference fixed at 470 here).",
