...
(function () {
  const I18N = {
    zh: {
      ...
      "world.term.e2e.ffmpeg": "ffmpeg 抽帧重标：边界相同，只把默认解码/抽帧换成 ffmpeg 路径。这不是新的标注策略，而是同一视频段的另一路候选描述；实验中常略高于默认原始帧重标，因此进入候选判别器的候选池。",
      ...
      "pipeline.e2e.analysis": "<p>结合上图与表，可以归纳如下。</p><ol><li>腕速一体基线：预测 810 段 / 参考 470 段，端到端整流程得分 0.0641，明显低于后续固定 S2 边界的路径。</li><li>分段模型自标：在同一 S2 边界上沿用分段描述，得分为 0.1234，说明只换更好描述路径仍有空间。</li><li>原始帧重标 · 27B / 397B：默认原始帧重标下，27B 为 0.1285，段内缩进 0.5 秒为 0.1337；397B 为 0.1414。同一方法下 397B 高于 27B，但增益主要来自噪声预测边界上的描述选择，而不是固定参考边界上的固定分段标注得分优势。</li><li>ffmpeg 抽帧重标 · 27B / 397B：27B 为 0.1440，397B 为 0.1491。换抽帧实现本身就能抬分，且两边同向；该路径常作为候选判别器的候选来源，而不是单独的新策略。</li><li>邻段重标 · 27B / 397B：27B 种子邻段重标 0.1260、原始帧先验邻段重标 0.1414；397B 先验邻段重标 0.1491。邻段上下文有时接近单路原始帧 / ffmpeg，但不稳定，且容易把邻段动作写进当前句。</li><li>多候选判别 · 27B / 397B：27B 候选判别器为 0.1362，低于同组最好的 27B ffmpeg；397B 候选判别器为 <strong>0.1542</strong>，语义匹配 60，仍为本组最高。WGO-Bench 公开的闭源最优端到端整流程得分为 0.168（全量约 100 集）；我们在 HomER 子集上的 0.1542 已与之接近。成本敏感场景可走单路原始帧 / ffmpeg；高精度场景保留 397B 候选判别器。</li></ol>",
      ...
      "story.e2e.6": "S2 边界 + 多候选判别 · 27B / 397B：在同一 S2 边界上生成多路候选描述，再分别由 27B 与 397B 候选判别器选出最终描述。重标提示词见 <a href=\"#label-prompt\">3.2</a>；候选判别器提示词见下方折叠。",
      ...
      "tldr.pt1": "<strong>先选对模型，再考虑复杂的视觉输入增强。</strong>给定人工分段、只评估片段描述时，Qwen3.6-27B 使用原始帧取得 55.7% 的固定分段标注得分；改用多帧拼贴图或叠加启发式视觉提示后，得分分别降至 52.8% 和 50.6%。在输入和提示词完全相同的分段实验中，Qwen3.6-27B 的视频分段得分为 0.1278，高于 Qwen3.5-397B-A17B 的 0.0952。就本组实验而言，模型选择带来的收益大于额外的视觉输入技巧。",
      ...
      "tldr.pt5": "<strong>单路原始帧标注已接近多候选判别方案，且成本更低。</strong>在相同预测边界上，使用 Qwen3.5-397B-A17B 进行单路原始帧标注，端到端整流程得分为 <strong>0.1414</strong>；生成多条描述并由候选判别器选出最终描述后，得分为 <strong>0.1542</strong>，相对提高约 9.1%。前者约 <strong>$1.1 / 视频小时</strong>，后者约 <strong>$2.7 / 视频小时</strong>。对成本敏感的批量处理，单路原始帧标注是更合适的默认方案。",
      ...
      "cost.row.wgo_scope": "原始帧单路 / 候选判别器工程估计",
      "cost.row.wgo_num": "原始帧约 $0.84–$1.59/h；候选判别器约 $2.11–$3.80/h",
      ...
      "cost.th.sel": "多候选 + 候选判别器（E2E 0.1542）",
      ...
      "cost.dyn.prod_p": "该路径与拼贴图 + 候选判别器路径不同；公开页只保留聚合口径。",
      "cost.dyn.api": "API 请求",
      ...
      "world.term.9": "候选判别器：对同一预测边界生成原始帧、ffmpeg、种子候选、先验描述等多路候选，再选出最像完成操作的一句；只在描述生成阶段使用。Gemini 评判下最高观察端到端整流程得分为 0.1542（397B 候选判别器）。",
      ...
      "fig.src.recipe": "assets/explain/recipe_flow_zh.svg?v=20260812-25",
      ...
      "walk.s3.t": "局部精修：窗口不外扩，盖住完整动作",
      "walk.s3.p": "在<strong>粗边界附近</strong>开局部时间窗，用<strong>同一版式</strong>的拼贴图在窗内再切一次。<strong>窗口不外扩</strong>：只看该局部时间窗内部；并加入「<strong>盖住完整动作</strong>」相关提示词。",
      ...
      "walk.s5.result": "候选判别器选出",
      "labelcmp.h2": "最终标注对照",
      "labelcmp.lead": "下面直观展示最终标注结果与原始人工标注的对比情况。上方播视频，下方两条时间轴；点击色块可查看该段标注并跳转播放。",
      "labelcmp.kicker": "最终标注对照",
      "conclusion.h2": "5. 结论",
      "conclusion.body": "<p>我们呈现了一条面向开放权重视觉语言模型的端到端第一视角标注管线 EgoANT，并在公开基准上测试了若干辅助 VLM 标注的输入与流程设计（例如拼贴图粗分、局部精修、多候选判别）。</p><p>与此同时，当前方案仍有两点潜在风险与局限，也留给后续继续探索：</p><ol><li><strong>模型能力并不整齐。</strong>不同模型在空间理解、推理、物体识别等方面强弱不一，因此没有哪种辅助技巧能长期通吃。好消息是，换成更新、更强的模型往往能直接提分；Macrodata 的报告里也有类似观察。</li><li><strong>基准得分锚定的是这份人工标注。</strong>WGO-Bench 的分数建立在人工分段与标注之上：它未必对应训练上最优的监督，人工标注本身也不一定质量最高。围绕该得分优化，会让管线趋近这份基准的人工标注风格。好消息是，如果未来有经过审核、规模更大、质量更好的人工标注数据，同一套得分规则与管线优化思路仍然适用。</li></ol>",
      "boundary.lead": "上方播视频，下方多轨时间轴；播放头随时间移动，点击色块可跳到该段。",
      "walk.s6.t": "点选预测段落：看视频 + 标注",
      "walk.s6.p": "这里只展示<strong>候选判别器预测轨</strong>：点击色块或表格行后，右侧播放对应片段，左侧显示预测标签。人工参考 / 整集粗分 / 拼贴图粗分的多轨边界对照见 <a href=\"#walk-boundary\">本节边界对照</a>。",
      ...
      "story.seg.modelnote": "条件名中的「· 27B / · 397B」分别表示该次调用使用 Qwen3.6-27B / Qwen3.5-397B-A17B。图中两侧并不对等：397B 只覆盖拼贴图分片、S1 加密切分、早期 S2（窗外扩约 1 秒）；整集一次提交、窗口外扩消融、覆盖完整动作提示词与相邻片段规则合并均只在 27B 上跑完。",
      ...
      "story.e2e.p": "固定 S2 分段边界后，只改变语义标注路径。<strong>27B 自标</strong>为 0.1234；<strong>27B 原始帧 · 段内缩进 0.5 秒</strong>为 0.1337；<strong>27B ffmpeg 抽帧</strong>为 0.1440；<strong>27B 候选判别器</strong>为 0.1362。也就是说，27B 在固定参考边界的固定分段标注得分上最高，但在噪声预测边界上的描述选择仍弱于 397B 候选判别器。<strong>397B 原始帧重标</strong>为 0.1414；<strong>397B ffmpeg 抽帧</strong>和<strong>397B 先验邻段重标</strong>均为 0.1491；<strong>397B 候选判别器</strong>从多路候选中选择最终标签，取得最高观察值 <strong>0.1542</strong>。该增益应与额外候选生成和候选判别器调用成本一起理解。",
      "story.e2e.fold": "展开：E2E 实验细节（做法卡）",
      "story.takeaway": "<strong>要点：</strong>分片请求边界会引入伪边界；分段规则定义要切分的完成动作，局部精修决定边界粒度；三种规则合并策略均降低视频分段得分；Gemini 重评后，原始帧 · 27B 是固定参考边界下观察值最高的标注配置，但端到端最高观察值仍来自 S2 预测边界 + 397B 多候选判别器。换句话说，<strong>27B 是最佳固定边界标注器，但不是最佳噪声预测片段描述选择器</strong>。视觉提示叠加、邻段拼贴和近似手部拼贴（含 27B 补评）目前不作为默认路径。",
      "th.cond": "条件",
      "th.model": "模型或方法",
      "th.note": "结果一句话",
      "th.acc": "固定分段标注得分",
      ...
      "recipe.r2.a": "低成本配置",
      "recipe.r2.b": "固定参考边界诊断：27B 原始帧（固定分段标注得分 55.7%）；预测边界单路配置：27B ffmpeg 抽帧（0.1440）或 397B 原始帧（0.1414，成本/部署不同）",
      "recipe.r2.c": "proxy overlay / neighbor sheet / whole-frame collage / proxy hand-crop 作为默认配置",
      "recipe.r3.a": "高准确率配置",
      "recipe.r3.b": "同一 S2 预测边界 + 多候选生成 + 397B 候选判别器（Gemini 端到端整流程得分 0.1542）",
    ...
    en: {
      ...
      "tldr.pt4": "<strong>Segmentation is the main bottleneck for full-pipeline scores.</strong> The best setup predicts 308 segments, but only 79 form valid temporal matches with the 470 human references; 60 of those also pass the semantic judge. Most losses happen before description quality is judged, so better action boundaries still offer more headroom than polishing labels.",
      "tldr.pt5": "<strong>Single-pass raw-frame labeling is close to multi-candidate selection at substantially lower cost.</strong> On the same predicted boundaries, a single Qwen3.5-397B-A17B raw-frame pass reaches E2E F1 <strong>0.1414</strong>. Generating multiple descriptions and using a candidate selector reaches <strong>0.1542</strong>, a 9.1% relative gain. Estimated costs are about <strong>$1.1 / video-hour</strong> and <strong>$2.7 / video-hour</strong>, respectively. For cost-sensitive batch processing, the single-pass path is the more practical default.",
      ...
      "fig.src.recipe": "assets/explain/recipe_flow.svg?v=20260812-25",
      ...
      "tldr.k.label": "Labeling score (Label Acc)",
      ...
      "walk.s3.t": "Local refinement: no pad-out, cover full actions",
      "walk.s3.p": "Open a local time window <strong>around a coarse boundary</strong> and re-cut inside it with the <strong>same collage layout</strong>. <strong>No pad-out</strong>: look only inside that local window; also add the related <strong>\"cover full actions\"</strong> prompt wording.",
      ...
      "recipe.r2.b": "Fixed-boundary diagnostic: 27B raw frames (labeling score 55.7%); predicted-boundary single-path: 27B ffmpeg frames (0.1440) or 397B raw frames (0.1414; different cost/deploy tradeoffs)",
