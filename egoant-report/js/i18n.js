...
(function () {
  const I18N = {
    zh: {
      ...
      "walk.s4.t": "多候选标注",
      "walk.s4.p": "分段边界固定后，对每个预测片段并行生成多路候选描述：<strong>原始帧</strong>、<strong>ffmpeg</strong>、<strong>种子候选</strong>、<strong>先验描述</strong>。下表是其中一个片段上的候选结果示例。",
      "walk.s5.t": "候选判别器选择",
      "walk.s5.p": "我们把上一步的多路候选交给模型，并输入提示词，让它从中选出最合适的动作标注。下面用本集预测段落互动展示选中结果：点击时间轴色块或表格行，可跳到该预测段落并查看候选。",
    ...
    en: {
      ...
      "walk.s4.p": "After segment boundaries are fixed, each predicted segment gets multiple candidate descriptions in parallel: <strong>raw frames</strong>, <strong>ffmpeg</strong>, <strong>seed</strong>, and <strong>prior</strong>. The table below shows candidates for one example segment.",
      "walk.s5.t": "Candidate selector",
      "walk.s5.p": "We send the previous candidates to the model with a prompt asking it to pick the most suitable action label. Below, click a timeline block or table row on this episode's predicted segments to jump there and inspect the candidates.",
