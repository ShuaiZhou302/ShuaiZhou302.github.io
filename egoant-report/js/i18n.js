...
(function () {
  const I18N = {
    zh: {
      ...
      "world.p.gepa": "GEPA 是一种根据反馈自动改写提示词的方法<sup class=\"cite\"><a class=\"cite-ref\" href=\"#ref-gepa\">3</a></sup>。Macrodata Labs 用它在独立的 15 集验证集上搜索更贴合标注规范的分段规则；本文复用其公开结果 <code>completed_events_duration_prior_v1</code>，没有重新运行 GEPA。使用时将完整视频的全部带时间戳拼贴图与规则文本一并提交，一次调用得到整集粗分。规则原文见案例分析 Step 02；概念与实现见 <a href=\"#app-seg\">附录 B</a>，下载见 <a href=\"#app-prompts\">附录 F</a>。",
      ...
      "world.term.4": "<strong>局部时间窗口</strong>：围绕某个粗边界截取的一小段连续视频区间，图中用绿色虚线框表示。",
      "world.term.5": "<strong>S1（加密切分配置）</strong>：仍对完整视频做一次分段，但通过修改提示词，要求模型输出更多、更短的候选片段。",
      "world.term.6": "<strong>S2（局部精修配置）</strong>：先由整集粗分得到粗边界，再围绕粗边界建立时间窗口，让模型在窗口内重新输出动作边界。图中绿色虚线只围住一个窗口；窗口内两块绿色表示重切后可能把原先欠分的区域拆得更细。",
      "world.term.7": "<strong>窗口不外扩（pad=0）</strong>：局部精修所用画面严格限制在时间窗口内。",
      "world.term.8": "<strong>覆盖完整动作（full-cover）</strong>：写入 S2 局部精修提示词的输出要求。窗口内从开始到结束都可见的操作事件都要切出；例如完整出现「拿起杯子」和「放下杯子」时应各输出一段，而仅有伸手接近或完成后收回手时，不单独成段。",
    ...
    en: {
      ...
      "world.term.4": "<strong>Local time window</strong>: a short continuous interval around one coarse boundary, shown by the dashed green box.",
      "world.term.5": "<strong>S1 (denser-cut configuration)</strong>: still segments the full video in one pass, but changes the prompt so the model returns more, shorter candidate segments.",
      "world.term.6": "<strong>S2 (local-refinement configuration)</strong>: a whole-episode pass first produces coarse boundaries; a window is then built around a coarse boundary, and the model predicts action boundaries again inside that window. The dashed green lines mark one window; the two green bars inside are finer segments after re-cutting an under-segmented region.",
      "world.term.7": "<strong>No pad-out (pad=0)</strong>: local refinement uses only frames inside the time window.",
      "world.term.8": "<strong>Cover full actions (full-cover)</strong>: an output requirement written into the S2 local-refinement prompt. Every manipulation event whose start and completion are both visible in the window should be cut out; if both \"pick up the cup\" and \"put down the cup\" are fully shown, output two segments, while merely reaching toward the cup or withdrawing the hand afterward should not form a segment on its own.",
