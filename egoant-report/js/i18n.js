...
(function () {
  const I18N = {
    zh: {
      ...
      "walk.s3.p": "在粗边界附近开局部时间窗，用<strong>同一版式</strong>的拼贴图再切一次。<strong>窗口不外扩</strong>：只看粗段内部；并加入「<strong>盖住完整动作</strong>」相关提示词。",
      "walk.s3.note": "以本集为例，整集粗分在约 <strong>26.0s</strong> 处得到「擦抽屉内部」粗段（约 <strong>[26.0, 42.0]</strong>）；下图是该粗段内一张局部拼贴图，覆盖 <strong>26.5s–36.0s</strong>（与上一步整集开头的拼贴图不同）。",
    ...
    en: {
      ...
      "walk.s3.p": "Open a local time window near the coarse bounds and re-cut with the <strong>same collage layout</strong>. <strong>No pad-out</strong>: look only inside the coarse segment; also add the related <strong>\"cover full actions\"</strong> prompt wording.",
      "walk.s3.note": "In this episode, whole-episode coarse cut places a \"wipe drawer interior\" coarse segment near <strong>26.0s</strong> (about <strong>[26.0, 42.0]</strong>); the figure below is one in-window collage covering <strong>26.5s–36.0s</strong> (different from the episode-start collage above).",
