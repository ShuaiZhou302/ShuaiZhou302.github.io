...
(function () {
  const I18N = {
    zh: {
      ...
      "conclusion.h2": "5. 结论",
      "conclusion.body": "<p>我们呈现了一条面向开放权重视觉语言模型的端到端第一视角标注管线 EgoANT，并在公开基准上测试了若干辅助 VLM 标注的输入与流程设计（例如拼贴图粗分、局部精修、多候选判别）。本页是 EgoANT blog 的第一个版本。</p><p>仍有两点局限，也留给后续继续探索：</p><ol><li><strong>模型能力并不整齐。</strong>不同模型在空间理解、推理、物体识别等方面强弱不一，因此没有哪种辅助技巧能长期通吃。好消息是，换成更新、更强的模型往往能直接提分；Macrodata 的报告里也有类似观察。</li><li><strong>基准得分锚定的是这份人工标注。</strong>WGO-Bench 的分数建立在人工分段与标注之上：它未必对应训练上最优的监督，人工标注本身也不一定质量最高。围绕该得分优化，会让管线趋近这份基准的人工标注风格。好消息是，如果未来有经过审核、规模更大、质量更好的人工标注数据，同一套得分规则与管线优化思路仍然适用。</li></ol>",
