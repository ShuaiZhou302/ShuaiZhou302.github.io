...
(function () {
  const I18N = {
    zh: {
      ...
      "world.p.rule": "<a href=\"https://microsoft.github.io/VITRA/\" target=\"_blank\" rel=\"noopener\">VITRA</a><sup class=\"cite\"><a class=\"cite-ref\" href=\"#ref-vitra\">5</a></sup> 将单目第一视角视频转换为三维手部与相机运动、原子动作片段和语言指令，并以腕部速度的局部极小值作为候选动作边界。另一类方法直接使用预设时长：Lin 等人的大行为模型协同训练研究<sup class=\"cite\"><a class=\"cite-ref\" href=\"#ref-cotraining\">10</a></sup>沿用 EgoVid 的划分方式，将筛选后的 Ego4D 视频切成固定 4 秒片段，再按 1 秒间隔抽帧生成描述。两类方法都不需要学习专门的边界预测器，但边界未必对应动作真正完成的时刻。基于运动低谷的方案尤其容易<strong>过分割</strong>：动作内部的停顿、重新抓握和手腕微调都可能产生候选边界，使「把胡萝卜放进碗里」这样的简单原子动作也被切成五六段。本文也探索了这一路线，以 HaWoR 腕速低谷生成边界并合并相邻片段；在 HomER 上，该方法预测 810 段，而人工参考为 470 段，Segment F1 为 <strong>0.0953</strong>，印证了过分割问题。为使边界更多地对应语义完整的动作，最终管线改用带时间戳的 contact sheet 进行分段。",
