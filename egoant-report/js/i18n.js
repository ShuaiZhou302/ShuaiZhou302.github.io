...
(function () {
  const I18N = {
    zh: {
      ...
      "world.term.1": "<strong>参考分段（gold）</strong>：人工标注的完整动作片段，图中用黄色表示。",
      ...
      "world.term.5": "<strong>S1（加密切分配置）</strong>：后文消融里对「加密切分」这一配置的简称：仍对完整视频做一次分段，但要求输出更多、更短的候选片段。",
      ...
      "world.term.8": "<strong>覆盖完整动作（full-cover）</strong>：写入 S2 局部精修提示词的输出要求，不是评测指标。它规定窗口内从开始到结束都可见的操作事件都要切出；例如完整出现「拿起杯子」和「放下杯子」时应各输出一段，而仅有伸手接近或完成后收回手时，不单独成段。",
      ...
      "world.zoo.collage": "<strong>时序拼贴</strong>：将同一片段内按时间顺序抽取的几帧拼成一张图，再一次性提交。",
      "world.zoo.contact": "<strong>拼贴图</strong>：每 0.5 秒抽取一帧，缩放至 224×144，每 20 帧组成一张网格，并在每格左上角标注黄色时间戳；一张约覆盖 10 秒。",
      "world.zoo.neighbor": "<strong>邻段拼贴</strong>：把上一段（灰条）、当前段（绿条）、下一段（灰条）的抽样帧一并提交，提供更多上下文。",
      ...
      "world.zoo.note": "以上示例只说明输入组织形式；对应实验结果见 <a href=\"#story\">§7.2</a>。",
    ...
    en: {
      ...
      "world.term.1": "<strong>Reference segments (gold)</strong>: complete actions annotated by humans and shown in yellow.",
      ...
      "world.term.5": "<strong>S1 (denser-cut configuration)</strong>: shorthand used later for an ablation that still segments the full video in one pass, but asks for more, shorter candidate segments.",
      ...
      "world.zoo.collage": "<strong>Temporal collage</strong>: a few frames sampled in temporal order from the same segment and tiled into one image.",
      ...
      "world.zoo.neighbor": "<strong>Neighbor collage</strong>: sampled frames from the previous (gray bar), current (green bar), and next (gray bar) segments are submitted together for extra context.",
