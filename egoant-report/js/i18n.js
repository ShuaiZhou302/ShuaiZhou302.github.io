...
(function () {
  const I18N = {
    zh: {
      ...
      "tldr.pt2": "<strong>最佳分段方案是「整集粗分 + 局部精修」。</strong>系统先用时间戳网格浏览完整视频并给出粗边界，再在每个粗边界附近重新判断动作起止。相较腕部速度规则基线，这一方案将分段得分从 0.0953 提升至 <strong>0.2031</strong>，并在 470 个人工参考片段中找到 79 个合格的时间匹配。Macrodata 公布的同范围 Gemini 参考值为 0.227；两者相差 0.024，但该数字来自外部报告，并非同一环境下的直接对照。",
      ...
      "tldr.seg": "79 / 470 个参考片段达到时间匹配标准。",
      "tldr.seg.config": "配置：Qwen3.6-27B · 时间戳网格 · 整集粗分 + 局部精修",
      "tldr.label": "262 / 470 个片段的描述通过语义评判。",
      "tldr.label.config": "配置：Qwen3.6-27B · 原始帧 · 固定人工分段",
      "tldr.e2e": "60 个片段同时通过时间匹配与语义评判。",
    ...
    en: {
      ...
      "tldr.pt1": "<strong>Choose the right model before adding complex visual-input augmentations.</strong> With human segments fixed and only descriptions evaluated, Qwen3.6-27B scores 55.7% using raw frames. Multi-frame collages and heuristic visual overlays reduce the score to 52.8% and 50.6%. In a segmentation comparison with identical timestamped grids and prompts, Qwen3.6-27B scores 0.1278, above Qwen3.5-397B-A17B at 0.0952. In these experiments, model choice contributes more than additional visual-input techniques.",
      "tldr.pt2": "<strong>The best segmentation recipe is a coarse whole-episode pass followed by local refinement.</strong> The model first scans timestamped grids for the complete video, proposes coarse boundaries, and then re-evaluates each local boundary. This raises Segment F1 from 0.0953 for the wrist-speed rule baseline to <strong>0.2031</strong>, yielding 79 valid temporal matches among 470 human reference segments. Macrodata reports 0.227 for Gemini on the same subset, a difference of 0.024, although that figure is an external reference rather than a controlled run in this environment.",
      ...
      "tldr.seg": "79 / 470 reference segments meet the temporal-match threshold.",
      "tldr.seg.config": "Setup: Qwen3.6-27B · timestamped grids · whole-episode pass + local refinement",
      "tldr.label": "262 / 470 segment descriptions pass the semantic judge.",
      "tldr.label.config": "Setup: Qwen3.6-27B · raw frames · fixed human segments",
      "tldr.e2e": "60 segments pass both temporal matching and semantic judging.",
