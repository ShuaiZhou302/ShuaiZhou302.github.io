...
(function () {
  const I18N = {
    zh: {
      ...
      "pipeline.seg.analysis": "<p>结合上图与表可见，多种不同方法呈现了不同的表现：</p><ol><li>腕速规则切段并合并：预测片段远多于参考分段（810 vs 470），说明运动低谷信号难以对齐动作语义边界。</li><li>拼贴图分片（每次最多 3 张）：一次请求只能看到局部时间范围，请求之间的接缝常被模型误判为动作边界。同设置下，Qwen3.5-397B 的视频分段 F1 为 0.0952，Qwen3.6-27B 为 0.1278；分片提交本身限制了两种模型的得分上限。</li><li>整集一次提交（无切段规则清单）：去掉分片接缝后伪边界减少，但预测片段偏少（148）。</li><li>整集一次提交 + 切段规则清单：比无规则清单更好，整体仍偏保守。</li><li>S1 加密切分：参考找回率上升（配对成功 80），同时预测段数增至 558，切分更细。</li><li>S2 局部精修：是主要增益来源。窗口不外扩优于窗外扩；「窗口不外扩 + 覆盖完整动作提示词」优于未加入该提示词，也优于同设置下的算法补覆盖。</li><li>S2 + 相邻片段规则合并：三种策略都接在上述最优 S2 预测之后做规则后处理，得分均低于未合并结果，因此不作为默认配置。</li><li>模型对比：同一拼贴图分片设置下，较新的 Qwen3.6-27B 视频分段 F1 为 0.1278，高于 Qwen3.5-397B 的 0.0952；最终最优 S2 配置也使用 27B。说明在本组实验里，更新、更先进的模型表现更好。</li></ol>",
      ...
      "story.seg.6": "S2 局部精修：先由整集粗分得到粗边界，再围绕每个粗边界开局部时间窗，把窗内拼贴图交回模型重切。「完整动作覆盖提示词」要求窗口内看得见起止的完成操作都要切出（见术语约定）。得分表中的变体包括：窗外扩约 1 秒、窗口不外扩（未加入完整动作覆盖提示词）、窗外扩 0.5/1/2 秒；另在「未加入完整动作覆盖提示词」的预测上，用脚本按中点规则事后补边界，记为「算法补覆盖」；最优设置为窗口不外扩 + 覆盖完整动作提示词（精修提示词见 <a href=\"#app-prompts\">附录 F</a> · <a href=\"prompts/s2_fullcover_refine.md\" download>下载</a>）。",
      ...
      "story.seg.legend": "表头：P（预测命中率）= 配对成功数 / 预测段数；R（参考找回率）= 配对成功数 / 参考分段数；match / pred / gold = 配对成功数 / 预测段数 / 参考分段数（本子集参考分段恒为 470）。若原始记录未单独给出 P/R，则按上式由 match / pred / gold 计算。",
      "th.segf1": "视频分段 F1",
    ...
    en: {
      ...
      "pipeline.seg.analysis": "<p>Reading the chart and table together, the different methods show different outcomes:</p><ol><li>Wrist-speed rule cuts + merge: far more predictions than reference segments (810 vs 470), so motion minima are a poor match to action semantics.</li><li>Chunked contact sheets (max 3 per call): each request sees only a local time span, and seams between requests are often mistaken for action boundaries. Under the same setting, video-segmentation F1 is 0.0952 for Qwen3.5-397B and 0.1278 for Qwen3.6-27B; chunked submission itself caps both runs.</li><li>Whole-episode request (no segmentation-rule list): seams disappear, but predictions are too few (148).</li><li>Whole-episode request + segmentation-rule list: better than the no-rule prompt, still conservative overall.</li><li>S1 denser cuts: reference recall rises (80 matches), with predictions growing to 558 and finer cuts.</li><li>S2 local refinement is the main gain. No pad-out beats pad-out; “no pad-out + cover-full-actions prompt” beats the no-prompt setting and also beats algorithmic cover under the same window.</li><li>S2 + adjacent-segment rule merges: all three strategies are rule postprocesses after that best S2 prediction and score below the unmerged result, so they are not defaults.</li><li>Model comparison: under identical chunked contact sheets, the newer Qwen3.6-27B reaches video-segmentation F1 0.1278, above Qwen3.5-397B at 0.0952; the best S2 setting also uses 27B. In this suite, the more recent, stronger model performs better.</li></ol>",
      ...
      "story.seg.legend": "Headers: P (precision / prediction hit rate) = matches / predicted segments; R (recall / reference recovery rate) = matches / reference segments; match / pred / gold = matches / predicted segments / reference segments (reference segments fixed at 470 here). If a row has no stored P/R, they are computed from match / pred / gold.",
