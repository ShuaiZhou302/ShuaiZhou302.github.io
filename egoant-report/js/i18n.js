...
(function () {
  const I18N = {
    zh: {
      ...
      "pipeline.seg.analysis": "<p>结合上图与表可见，多种不同方法呈现了不同的表现：</p><ol><li>腕速规则切段并合并：预测片段远多于参考分段（810 vs 470），视频分段 F1 仅 0.0953，说明运动低谷信号难以对齐动作语义边界。</li><li>拼贴图分片（每次最多 3 张）：一次请求只能看到局部时间范围，请求之间的接缝常被模型误判为动作边界。同设置下，视频分段 F1 分别为 Qwen3.5-397B 的 0.0952 与 Qwen3.6-27B 的 0.1278；分片提交本身限制了两种模型的得分。</li><li>整集一次提交（无切段规则清单）：去掉分片接缝后伪边界减少，但预测片段偏少（148），视频分段 F1 为 0.1230。</li><li>整集一次提交 + 切段规则清单：视频分段 F1 升至 0.1369，比无规则清单更好，整体仍偏保守。</li><li>S1 加密切分：参考找回率上升（配对成功 80），视频分段 F1 到 0.1556，同时预测段数增至 558，切分更细。</li><li>S2 局部精修：是主要增益来源。窗外扩约 1 秒时视频分段 F1 为 0.1674；窗口不外扩但未写入覆盖完整动作约束时为 0.1711，窗外扩 0.5/1/2 秒均更低；算法补覆盖为 0.1635，不及把覆盖完整动作写进提示词。最终「窗口不外扩并覆盖完整动作」取得最高视频分段 F1 0.2031。</li><li>S2 + 相邻片段规则合并：三种策略都接在上述最优 S2 预测之后做规则后处理，视频分段 F1 分别降为 0.1987 / 0.1947 / 0.1883，因此不作为默认配置。</li><li>模型对比：同一拼贴图分片设置下，较新的 Qwen3.6-27B 视频分段 F1 为 0.1278，高于 Qwen3.5-397B 的 0.0952；最终最优 S2 配置也使用 27B。说明在本组实验里，更新、更先进的模型表现更好。</li></ol>",
      ...
      "story.seg.6": "S2 局部精修：先由整集粗分得到粗边界，再围绕每个粗边界开局部时间窗，把窗内拼贴图交回模型重切。这里的「覆盖完整动作」指提示词要求：窗口内看得见起止的完成操作都要切出（见术语约定）。得分表中的变体包括：窗外扩约 1 秒、窗口不外扩（未写入覆盖完整动作约束）、窗外扩 0.5/1/2 秒、窗口不外扩 + 算法补覆盖（不用该提示词，改用脚本按中点规则事后补边界），以及窗口不外扩并覆盖完整动作（精修提示词见 <a href=\"#app-prompts\">附录 F</a> · <a href=\"prompts/s2_fullcover_refine.md\" download>下载</a>）。",
      "story.seg.7": "S2 + 相邻片段规则合并：先在已有预测边界上，用规则脚本合并相邻片段，不再调用模型。本组实验接在 S2「窗口不外扩并覆盖完整动作」之后；脚本分别按标签完全相同、动词/物体相同，或跨短间隙桥接后再合并。",
    ...
    en: {
      ...
      "pipeline.seg.analysis": "<p>Reading the chart and table together, the different methods show different outcomes:</p><ol><li>Wrist-speed rule cuts + merge: far more predictions than reference segments (810 vs 470) and only 0.0953 video-segmentation F1, so motion minima are a poor match to action semantics.</li><li>Chunked contact sheets (max 3 per call): each request sees only a local time span, and seams between requests are often mistaken for action boundaries. Under the same setting, video-segmentation F1 is 0.0952 for Qwen3.5-397B and 0.1278 for Qwen3.6-27B; chunked submission itself caps both runs.</li><li>Whole-episode request (no segmentation-rule list): seams disappear, but predictions are too few (148) and video-segmentation F1 is 0.1230.</li><li>Whole-episode request + segmentation-rule list: video-segmentation F1 rises to 0.1369, better than the no-rule prompt, still conservative overall.</li><li>S1 denser cuts: reference recall rises (80 matches) and video-segmentation F1 reaches 0.1556, with predictions growing to 558 and finer cuts.</li><li>S2 local refinement is the main gain. ≈1s pad-out reaches video-segmentation F1 0.1674; no pad-out without the cover-full-actions prompt reaches 0.1711, while 0.5/1/2s pad-out are all worse; algorithmic cover reaches 0.1635 and underperforms writing cover-full-actions into the prompt. The final no-pad-out + cover-full-actions setting reaches the best video-segmentation F1 of 0.2031.</li><li>S2 + adjacent-segment rule merges: all three strategies are rule postprocesses after that best S2 prediction and drop video-segmentation F1 to 0.1987 / 0.1947 / 0.1883, so they are not defaults.</li><li>Model comparison: under identical chunked contact sheets, the newer Qwen3.6-27B reaches video-segmentation F1 0.1278, above Qwen3.5-397B at 0.0952; the best S2 setting also uses 27B. In this suite, the more recent, stronger model performs better.</li></ol>",
      ...
      "story.seg.6": "S2 local refinement: take coarse bounds from the whole-episode pass, open a local time window around each bound, and re-cut from the in-window contact sheets. “Cover full actions” means the prompt requires every completed operation whose start and end are visible in the window to be cut out (see Terminology). Score-table variants include ≈1s pad-out, no pad-out without that prompt constraint, 0.5/1/2s pad-out, no pad-out + algorithmic cover (scripted midpoint cover instead of the prompt), and no pad-out + cover full actions (refine prompt: <a href=\"#app-prompts\">Appendix F</a> · <a href=\"prompts/s2_fullcover_refine.md\" download>download</a>).",
      "story.seg.7": "S2 + adjacent-segment rule merges: first apply rule scripts on existing predicted boundaries, without another model call. In this report the scripts run after S2 “no pad-out + cover full actions”; they merge by identical labels, shared verb/object, or short-gap bridging.",
