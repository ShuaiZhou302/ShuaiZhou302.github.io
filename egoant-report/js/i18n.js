...
(function () {
  const I18N = {
    zh: {
      ...
      "pipeline.label.analysis": "<p>结合上图与表，可以归纳如下。</p><ol><li>原始帧 · 27B：在 470 个固定参考片段上配对成功 262，固定分段标注得分 55.7%，为本组最高。同一评测尺子下，复杂视觉输入均未超过该配置。</li><li>时序拼贴与视觉提示叠加：在 27B 上分别得到 52.8% 与 50.6%；在 397B 上分别得到 45.1% 与 48.5%，均低于对应模型的原始帧。额外上下文没有提高当前片段描述准确率。</li><li>HaWoR 手部裁剪：27B 为 52.3%（246 / 470），高于 397B 的 50.9%，但仍低于原始帧 · 27B。裁剪能略微帮助部分模型，但不能替代原始帧和模型选择。</li><li>邻段拼贴：27B 为 40.9%，397B 为 39.6%，397B 加秒级时间戳后为 40.0%。换模型只带来很小变化，相邻动作仍容易被写进当前描述。</li><li>近似手部拼贴：27B 为 38.5%，397B 为 39.1%。这里的近似手部拼贴使用 YOLO 或画面中心启发式裁剪，不读取 HaWoR 腕轨迹，因此不等同于真正的手部重建裁剪。</li><li>模型对比：原始帧输入下，27B（55.7%）高于 397B（50.2%）。固定边界标注的默认路径取原始帧 · 27B。</li></ol>",
      ...
      "story.label.1": "原始帧 · 27B：用上文「原始帧」，由 Qwen3.6-27B 生成描述。",
      "story.label.2": "时序拼贴 · 27B：用上文「时序拼贴」，交给 Qwen3.6-27B。",
      "story.label.3": "视觉提示叠加 · 27B：用上文「视觉提示叠加」后标注。",
      "story.label.4": "原始帧 · 397B：同上文「原始帧」，改用 Qwen3.5-397B。",
      "story.label.5": "视觉提示叠加 · 397B：同上文「视觉提示叠加」，改用 Qwen3.5-397B。",
      "story.label.6": "时序拼贴 · 397B：同上文「时序拼贴」，改用 Qwen3.5-397B。",
      "story.label.7": "邻段拼贴 · 27B / 397B：用上文「邻段拼贴」，分别由 Qwen3.6-27B 与 Qwen3.5-397B 生成描述。",
      "story.label.8": "邻段拼贴 + 秒级时间戳 · 397B：在上文「邻段拼贴」上为每格加入秒级时间戳后重跑。",
      "story.label.9": "近似手部拼贴 · 27B / 397B：用 YOLO 或画面中心启发式裁剪手部区域后拼图提交；这不是 HaWoR 腕轨迹裁剪。",
      "story.label.10": "HaWoR 手部裁剪 · 397B：用上文「手部裁剪」（HaWoR 腕轨迹）；裁剪不完整时回退到原始帧。",
      ...
      "world.input.6": "手部裁剪：按估计腕轨迹裁剪手部区域后提交。",
      ...
      "contact.taxonomy.explain": "<strong>读图：</strong>contact sheet 用于时间分段；raw frames 是固定边界标注的基础输入；proxy overlay 是在原帧上叠加光流或启发式提示，不是手部重建；temporal collage 与 neighbor sheet 引入前后文；手部相关输入分为两类：proxy hand-collage 是 YOLO 或画面中心启发式近似裁剪，HaWoR wrist-guided crop 才读取重建腕轨迹。Gemini 重评显示，增加视觉上下文并未提高 HomER 上的固定边界标注准确率。",
      ...
      "th.segf1": "视频分段得分",
      "story.label.p": "固定参考边界后，Gemini judge 全量重评显示：<strong>raw 27B 的观察值最高，为 55.7%</strong>；temporal collage 27B 为 52.8%，HaWoR wrist-guided crop 27B 为 52.3%，proxy overlay 27B 为 50.6%，HaWoR wrist-guided crop 397B 为 50.9%，raw 397B 为 50.2%。新补的 27B 邻段拼贴为 40.9%，27B 近似手部拼贴为 38.5%；它们没有改变结论。需要区分两类手部输入：proxy hand-collage 是启发式近似裁剪，HaWoR wrist-guided crop 才读取重建腕轨迹。",
      "story.label.cap1": "读图（三栏）：左为原始帧；中为启发式框（画面中心偏下固定方框，不是手腕检测）；右为送给标注模型的裁剪图。该近似裁剪路径在固定边界上低于原始帧。见 <a href=\"#term-visual\">视觉输入</a>。",
      "story.label.cap2": "读图（三栏）：左为原始帧；中为 YOLO 人物框；右为模型实际看到的裁剪。早期无手重建时使用此类近似输入；真正读取 HaWoR 腕轨迹的手部裁剪在固定分段上达到 52.3%（27B）/ 50.9%（397B），仍低于 raw 27B。",
      "story.label.fold": "展开：标注实验细节（做法卡）",
      "story.e2e.p": "固定 S2 分段边界后，只改变语义标注路径。<strong>27B 自标</strong>为 0.1234；<strong>27B raw inner-0.5</strong> 为 0.1337；<strong>27B ffmpeg raw</strong> 为 0.1440；<strong>27B selector</strong> 为 0.1362。也就是说，27B 在固定参考边界 Label Acc 上最高，但在噪声预测边界上不是最强定稿器。<strong>397B raw 重标</strong>为 0.1414；<strong>397B ffmpeg raw</strong> 和 <strong>397B-prior neighbor</strong> 均为 0.1491；<strong>397B candidate selector</strong> 从多路候选中选择最终标签，取得最高观察值 <strong>0.1542</strong>。该增益应与额外候选生成和 selector 调用成本一起理解。",
      "story.e2e.fold": "展开：E2E 实验细节（做法卡）",
      "story.takeaway": "<strong>要点：</strong>分片请求边界会引入伪边界；分段规则定义要切分的完成动作，局部精修决定边界粒度；三种规则合并策略均降低视频分段得分；Gemini 重评后，raw 27B 是固定参考边界下观察值最高的标注配置，但端到端最高观察值仍来自 S2 预测边界 + 397B 多候选 selector。换句话说，<strong>27B 是最佳 fixed-boundary captioner，但不是最佳 noisy predicted-segment label resolver</strong>。proxy overlay、neighbor sheet 和 proxy hand-collage（含 27B 补评）目前不作为默认路径。",
      "th.cond": "条件",
      "th.model": "模型或方法",
      "th.note": "结果一句话",
      "th.acc": "固定分段标注得分",
      ...
      "recipe.r2.a": "低成本配置",
      "recipe.r2.b": "固定参考边界诊断：27B raw（Label Acc 55.7%）；预测边界单路配置：27B ffmpeg raw（0.1440）或 397B raw（0.1414，成本/部署不同）",
    ...
    en: {
      ...
      "pipeline.label.analysis": "<p>Reading the chart and table yields the following summary.</p><ol><li>Raw frames · 27B: 262 of 470 fixed reference segments match, for Label Acc 55.7%, the highest in this group. Under the same judge, no richer visual input exceeds this setting.</li><li>Temporal collage and visual-hint overlay: 52.8% and 50.6% on 27B; 45.1% and 48.5% on 397B—all below the corresponding raw-frame runs. Extra context does not improve current-segment descriptions.</li><li>HaWoR hand crop: 27B reaches 52.3% (246 / 470), above the 397B hand-crop row at 50.9%, but still below raw 27B. Cropping helps some cases, but it does not replace raw frames and model choice.</li><li>Neighbor collage: 27B reaches 40.9%, 397B reaches 39.6%, and 397B with second-level timestamps reaches 40.0%. Changing the model barely changes this pattern; neighboring actions still leak into the current description.</li><li>Approximate hand collage: 27B reaches 38.5% and 397B reaches 39.1%. This path uses YOLO or center-heuristic crops, not HaWoR wrist tracks, so it should not be read as a true reconstructed hand-crop result.</li><li>Model comparison: with raw frames, 27B (55.7%) beats 397B (50.2%). The default fixed-boundary path is raw frames · 27B.</li></ol>",
      ...
      "story.label.1": "Raw frames · 27B: use the raw-frame input above; caption with Qwen3.6-27B.",
      "story.label.2": "Temporal collage · 27B: use the temporal-collage input above with Qwen3.6-27B.",
      "story.label.3": "Visual-hint overlay · 27B: use the visual-hint overlay above, then label.",
      "story.label.4": "Raw frames · 397B: same raw-frame input as above, labeled with Qwen3.5-397B.",
      "story.label.5": "Visual-hint overlay · 397B: same overlay input as above, labeled with Qwen3.5-397B.",
      "story.label.6": "Temporal collage · 397B: same temporal-collage input as above, labeled with Qwen3.5-397B.",
      "story.label.7": "Neighbor collage · 27B / 397B: use the neighbor-collage input above, labeled with Qwen3.6-27B and Qwen3.5-397B.",
      ...
      "story.label.9": "Approximate hand collage · 27B / 397B: crop hand-adjacent regions with YOLO or a center heuristic, then tile the crops; this is not HaWoR wrist-guided crop.",
      "story.label.10": "HaWoR hand crop · 397B: use the hand-crop input above (HaWoR wrist tracks); fall back to raw frames when crops are incomplete.",
      ...
      "world.input.6": "Hand crop: a hand region cropped by the estimated wrist track.",
      ...
      "contact.taxonomy.explain": "<strong>How to read it:</strong> contact sheets are used for temporal segmentation; raw frames are the basic fixed-boundary labeling input; proxy overlay means raw frames with optical-flow or heuristic marks, not hand reconstruction; temporal collages and neighbor sheets add context. Hand-related inputs are split: proxy hand-collage uses YOLO or center-heuristic approximate crops, while HaWoR wrist-guided crop reads reconstructed wrist tracks. The Gemini rescore shows that added visual context did not improve fixed-boundary label accuracy on HomER.",
      ...
      "story.label.cap2": "How to read (3 panels): left = raw frame; middle = YOLO person box; right = crop the model sees. Early runs without hand reconstruction used such approximate inputs; the true HaWoR wrist-guided crop reaches 52.3% with 27B and 50.9% with 397B, still below raw 27B.",
      "story.label.fold": "Expand: labeling method cards",
      "story.e2e.p": "With S2 segment boundaries fixed, only the semantic labeling path changes. <strong>27B self-label</strong> gives 0.1234; <strong>27B raw inner-0.5</strong> reaches 0.1337; <strong>27B ffmpeg raw</strong> reaches 0.1440; and <strong>27B selector</strong> reaches 0.1362. In other words, 27B has the best fixed-boundary Label Acc, but it is not the strongest finalizer under noisy predicted boundaries. <strong>397B raw relabel</strong> reaches 0.1414; <strong>397B ffmpeg raw</strong> and <strong>397B-prior neighbor</strong> both reach 0.1491; <strong>397B candidate selector</strong> chooses the final label from multiple candidates and obtains the highest observed score, <strong>0.1542</strong>. This gain should be interpreted together with the additional cost of candidate generation and selector calls.",
      ...
      "recipe.r2.b": "Fixed-reference-boundary diagnostic: 27B raw (Label Acc 55.7%); predicted-boundary single-path options: 27B ffmpeg raw (0.1440) or 397B raw (0.1414, different cost/deployment tradeoff)",
