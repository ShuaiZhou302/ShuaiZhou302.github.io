...
(function () {
  const I18N = {
    zh: {
      ...
      "world.zoo.proxy": "<strong>视觉提示叠加 / proxy overlay</strong>：在完整帧上叠加估计的手部或运动提示点与短时轨迹后提交。有手部重建时可画腕部轨迹；没有手部重建时使用 optical-flow proxy。本图使用同一 HomER 样本段的 proxy 可视化。",
      "world.zoo.crop": "<strong>手部裁剪</strong>：左图橙色框表示裁剪区域，右图是实际送入标注模型的局部图像。它与在完整画面上保留轨迹叠加的 proxy overlay 是两种不同输入。",
      ...
      "walk.h2": "5. 案例分析：homer_4 的端到端处理流程",
      ...
      "appendix.h2": "9. 附录：概念、公式、实现与成本记账",
      ...
      "walk.s6.p": "这里只展示 <strong>selector 预测轨</strong>：点击色块或表格行后，右侧播放对应片段，左侧显示预测标签。gold / 整集粗分 / contact-sheet 粗分的多轨边界对照见上方 <a href=\"#boundary\">§4.5</a>。",
      ...
      "nav.boundary": "边界对照",
      "boundary.h2": "4.5 边界对照：同一视频上的三种分段结果",
    ...
    en: {
      ...
      "world.zoo.proxy": "<strong>Visual-hint overlay / proxy overlay</strong>: estimated hand or motion cue points and short trails are drawn on the full frame before submission. With hand reconstruction this can use wrist tracks; without it, the pipeline uses an optical-flow proxy. This figure uses the same HomER sample segment.",
      ...
      "walk.h2": "5. Case study: end-to-end processing of homer_4",
      ...
      "appendix.h2": "9. Appendix: concepts, formulas, implementation, cost accounting",
      ...
      "walk.s6.p": "This section shows the <strong>selector prediction track</strong>: click a bar or table row to play the corresponding clip on the right and inspect its predicted label on the left. The multi-track comparison with gold, whole-episode coarse, and contact-sheet coarse boundaries appears in <a href=\"#boundary\">§4.5</a>.",
      ...
      "boundary.h2": "4.5 Boundary comparison: three segmentation results on one video",
