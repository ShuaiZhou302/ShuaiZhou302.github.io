...
(function () {
  const I18N = {
    zh: {
      ...
      "world.term.9": "候选判别器：对同一预测边界生成原始帧、ffmpeg、种子描述、原始帧先验等多路候选，再选出最像完成操作的一句；只在描述生成阶段使用。Gemini 评判下最高观察端到端整流程得分为 0.1542（397B 候选判别器）。",
      ...
      "walk.s4.t": "多候选标注",
      "walk.s4.p": "固定分段边界后，对每个预测片段生成多条候选描述：<strong>原始帧</strong>使用默认抽帧；<strong>ffmpeg</strong>使用另一套解码/抽帧路径；<strong>种子描述</strong>与<strong>原始帧先验</strong>使用先前模型输出作为文本先验。候选判别器在推理时只看候选及其来源，不看人工参考标签。",
      "walk.s5.t": "候选判别器选择",
    ...
    en: {
      ...
      "world.term.9": "Candidate selector: generate raw-frame, ffmpeg, seed-description, and raw-prior candidates for the same predicted boundary, then pick the sentence that best names the completed operation; used only during description generation. Under the Gemini judge, the highest observed E2E F1 is 0.1542 (397B candidate selector).",
      ...
      "walk.s4.p": "After segment boundaries are fixed, each predicted segment receives multiple candidate descriptions: <strong>raw frames</strong> use default sampling; <strong>ffmpeg</strong> uses an alternate decode/sampling path; <strong>seed</strong> and <strong>raw-prior</strong> use previous model outputs as textual priors. At inference time, the candidate selector sees candidates and their sources, not gold labels.",
