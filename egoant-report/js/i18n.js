...
(function () {
  const I18N = {
    zh: {
      ...
      "pipeline.e2e.lead": "端到端阶段固定较优分段边界，只改变标注路径，比较单路生成与多候选判别；分数为 E2E F1。",
      ...
      "pipeline.seg.analysis": "结合上图可以看出几条清晰趋势。腕速规则基线预测片段远多于人工参考，Segment F1 只有 0.0953，说明仅靠运动低谷很难对齐动作语义边界。拼贴图分片提交时，请求之间的接缝容易被当成动作边界，大模型分片版得分与腕速基线接近；同设置下较小模型略高，但仍受分片限制。改为整集一次提交后，这类接缝伪边界减少，但旧版提示词预测片段偏少；加入切段规则清单后得分升至 0.1369，仍偏保守。第一遍加密切分抬高了召回，也带来更多过细切分。第二遍局部精修是主要增益来源：在窗口不外扩、并在提示词中要求盖住完整动作时，Segment F1 达到已评测配置中的最高值 0.2031；把「盖住完整动作」写成算法后处理，效果不如写进提示词。三种相邻片段规则合并都会降低 Segment F1，因此不作为默认配置。",
      "pipeline.seg.methods": "我们比较过的分段做法如下。",
      "pipeline.seg.lead": "分段要解决的问题是：给定一段第一视角长视频，输出动作级片段的时间边界。输入是完整视频；实验中通常先按术语约定做成带时间戳的拼贴图序列，再交给模型或规则方法。输出是一组起止时间（例如 [12.0, 18.5]），本阶段只评边界是否与人工参考对齐。分数统一为同一 HomER 子集上的 Segment F1；下面先列尝试过的做法，数值见随后的得分图与表。",
      "pipeline.h3.seg": "4.1 分段：尝试过的方法与结果",
      "pipeline.h3.roles": "所用模型",
      ...
      "world.p.models": "管线实验会用到若干视觉语言模型。我们选用 Qwen3.6-27B 与 Qwen3.5-397B-A17B，主要因为它们是目前较常见、易于私有部署的开源模型，便于在分段策略与标注分工上做对照；下文各表会标明每一次消融实际调用了哪个模型，这里不预先固定「谁负责分段、谁负责定稿」。Gemini-3.5-Flash 不参与边界预测、描述生成或候选选择，仅用于离线评测：按 Macrodata Labs 的 <a href=\"https://huggingface.co/datasets/macrodata/WGO-Bench\" target=\"_blank\" rel=\"noopener\">WGO-Bench</a> 设定，用它对描述与人工标注做语义一致性打分（见 <a href=\"https://ai.google.dev/gemini-api/docs/pricing\" target=\"_blank\" rel=\"noopener\">Gemini 官方定价</a>）。",
      ...
      "story.seg.1": "腕速规则切段：用 HaWoR 从视频重建手部运动并估计腕部速度曲线，在速度局部低谷处放置候选切点，再对相邻片段做规则合并。这是原 EgoANT 生产管线中的启发式基线，不调用视觉语言模型。",
      "story.seg.2": "拼贴图分片提交：把整集拼贴图按请求拆开，每次最多送入 3 张，由模型只在该局部时间范围内预测边界。这样做外形接近公开报告中的输入形式，但一次请求只能看到视频的一段连续时间。",
      "story.seg.3": "整集一次提交 + 旧版提示词：把整集全部拼贴图放进同一次调用，不再分片；提示词尚未加入切段规则清单。",
      "story.seg.4": "整集一次提交 + 切段规则清单：仍是整集拼贴图一次提交，但提示词改用 Macrodata 用 GEPA 搜索得到的切段规则（本文复用其公开规则文本，见术语约定）。",
      "story.seg.5": "第一遍加密切分：仍对整集做一次分段，但通过修改提示词要求模型输出更多、更短的候选片段（术语中的 S1）。",
      "story.seg.6": "第二遍局部精修：先由整集粗分得到粗边界，再围绕每个粗边界开局部时间窗，把窗内拼贴图交回模型重切。我们比较了窗口是否向外扩展，以及是否在提示词中要求「盖住完整动作」；另测过用算法中点覆盖补全边界的后处理。",
      "story.seg.7": "相邻片段规则合并：在已有模型预测上，用脚本按时间相邻、动词相同或短间隙桥接等规则合并片段，不再调用模型。",
      "story.chart.seg": "Segment F1（主路径）",
      "story.seg.legend": "表头：P（Precision）= match / pred（预测段里配对成功的比例）；R（Recall）= match / gold（人工参考段被找回的比例）；match / pred / gold = 配对成功数 / 预测段数 / 人工参考段数（本子集人工参考恒为 470）。「模型」列若写「规则后处理」，表示在已有预测上做脚本合并，不再调用视觉语言模型。",
    ...
    en: {
      ...
      "pipeline.e2e.lead": "The end-to-end stage freezes a strong segmentation boundary and varies only the labeling path, comparing single-pass generation with multi-candidate selection. Scores are E2E F1.",
      ...
      "pipeline.seg.analysis": "The chart shows a few clear trends. The wrist-speed rule baseline predicts far more segments than the human reference and reaches only 0.0953 Segment F1, so motion minima alone are a poor match to action semantics. Chunked contact-sheet requests create seams that the model often treats as action boundaries; the larger-model chunked run stays near the wrist baseline, and the smaller model is only modestly better under the same chunking limit. Sending the whole episode in one call removes those request seams, but the legacy prompt under-predicts segments; adding the segmentation-rule list raises the score to 0.1369 while remaining conservative. Pass-1 denser cuts lift recall and also produce finer over-cuts. Pass-2 local refinement is the main gain: with no window pad-out and a prompt that requires covering completed actions, Segment F1 reaches the best evaluated value of 0.2031. Encoding the same cover constraint as algorithmic postprocessing underperforms writing it into the prompt. All three adjacent-segment rule merges lower Segment F1, so they are not used as defaults.",
      "pipeline.seg.methods": "The segmentation options we compared are as follows.",
      "pipeline.seg.lead": "Segmentation asks: given one long egocentric video, output action-level time boundaries. The input is the full video; in our experiments it is usually converted into a timestamped contact-sheet sequence, as defined in the terminology section, before a model or rule method sees it. The output is a set of start–end intervals (for example [12.0, 18.5]); this stage scores boundary alignment only. All scores are Segment F1 on the same HomER subset. Methods come first below; numbers follow in the chart and table.",
      "pipeline.h3.seg": "4.1 Segmentation: options and results",
      "pipeline.h3.roles": "Models used",
      ...
      "world.p.models": "The pipeline experiments use several vision-language models. We chose Qwen3.6-27B and Qwen3.5-397B-A17B mainly because they are widely used, easy-to-deploy open-weight models, which makes it practical to ablate segmentation strategies and labeling role splits; later tables name the model used in each run, so this section does not freeze a final “who segments / who finalizes” assignment. Gemini-3.5-Flash does not predict boundaries, generate descriptions, or select candidates. It is used only offline for semantic scoring, following Macrodata Labs’ <a href=\"https://huggingface.co/datasets/macrodata/WGO-Bench\" target=\"_blank\" rel=\"noopener\">WGO-Bench</a> judge setup (see <a href=\"https://ai.google.dev/gemini-api/docs/pricing\" target=\"_blank\" rel=\"noopener\">Gemini pricing</a>).",
      ...
      "walk.prompt.sel": "Prompt (English only) — selector",
      "walk.prompt.judge": "Prompt (English only) — judge (scoring only)",
      "story.seg.1": "Wrist-speed rule cuts: reconstruct hand motion with HaWoR, estimate the wrist-speed curve, place candidate cuts at local speed minima, then merge adjacent segments with rules. This is the original EgoANT production heuristic and does not call a vision-language model.",
      "story.seg.2": "Chunked contact-sheet requests: split the episode’s contact sheets across API calls, sending at most three sheets per call, so the model predicts boundaries only inside that local time span. The input format resembles public reports, but each request sees only a continuous slice of the video.",
      "story.seg.3": "Whole-episode request with the legacy prompt: send every contact sheet from the episode in one call, with no chunking; the prompt does not yet include the segmentation-rule list.",
      "story.seg.4": "Whole-episode request with the segmentation-rule list: keep the single whole-episode call, but replace the prompt with the GEPA-searched rule text released by Macrodata (this report reuses that public text; see Terminology).",
      "story.seg.5": "Pass-1 denser cuts: still segment the whole episode once, but change the prompt so the model returns more, shorter candidate segments (S1 in Terminology).",
      "story.seg.6": "Pass-2 local refinement: take coarse bounds from the whole-episode pass, open a local time window around each bound, and re-cut from the in-window contact sheets. We compared whether the window expands outward and whether the prompt requires covering completed actions; we also tested algorithmic midpoint-cover postprocessing.",
      "story.seg.7": "Adjacent-segment rule merges: on top of existing model predictions, merge segments with scripts based on temporal adjacency, shared verb/object, or short-gap bridging, without another model call.",
      "story.chart.seg": "Segment F1 (main path)",
      "story.seg.legend": "Headers: P (Precision) = match / pred; R (Recall) = match / gold; match / pred / gold = counts (human reference fixed at 470 here). If the model column says “rule postprocess”, it means scripted merges on existing predictions—no vision-language model call.",
