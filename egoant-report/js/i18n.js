...
(function () {
  const I18N = {
    zh: {
      ...
      "world.p.scale": "Scale Labs 的 <a href=\"https://labs.scale.com/blog/path-to-large-scale-dense-video-captioning\" target=\"_blank\" rel=\"noopener\">The Path to Large Scale Dense Video Captioning</a><sup class=\"cite\"><a class=\"cite-ref\" href=\"#ref-scale\">4</a></sup> 讨论在<strong>已经切分好的 clip</strong> 上生成稠密描述。其核心工程贡献之一是用 <strong>contact sheet 辅助标注</strong>：将多个稳定抽样帧拼成一张网格图，并结合时间信息提供紧凑的片段表示，从而减少逐帧输入的开销。该设计直接启发了本文的视觉输入形式；但它默认边界已经给定，不涉及从原始长视频中预测动作边界。",
      "world.h3.egoant": "EgoANT 的最终管线与模型分工",
      "world.p.egoant": "本文最终采用一条以视觉语言模型为核心的自动标注管线：<strong>带时间戳的 contact sheets → Qwen3.6-27B 分段 → 在预测边界上生成操作描述 → 可选的多候选判别</strong>。腕速规则方法仅作为分段消融与对照，不构成另一条 EgoANT 管线。",
