...
(function () {
  const I18N = {
    zh: {
      ...
      "pipeline.seg.lead": "分段阶段比较规则切分、拼贴图分片、整集粗分、加密切分与局部精修等路径；分数来自同一 HomER 子集上的 Segment F1。",
      "pipeline.h3.seg": "4.1 分段：尝试过的方法与结果",
      "pipeline.h3.roles": "模型角色",
      ...
      "pipeline.lead": "本节按管线顺序说明：先分段、再标注，最后看端到端串联结果。每个阶段先交代尝试过的方法，再给出对应实验结果；读完即可看出当前最优路径，并落到推荐配置与成本。",
      ...
      "world.p.models": "生成与评测的角色分工如下：Qwen3.6-27B 负责分段，也可生成部分候选描述；Qwen3.5-397B-A17B 负责生成候选描述并由候选判别器选出定稿；Gemini-3.5-Flash 仅在离线评测时判断描述与人工标注是否语义一致，不参与边界预测、描述生成或候选选择。",
    ...
    en: {
      ...
      "pipeline.seg.lead": "The segmentation stage compares rule-based cuts, chunked contact sheets, whole-episode coarse passes, denser cuts, and local refinement. Scores are Segment F1 on the same HomER subset.",
      "pipeline.h3.seg": "4.1 Segmentation: options and results",
      "pipeline.h3.roles": "Model roles",
      ...
      "pipeline.lead": "This section follows the pipeline: segmentation, then labeling, then end-to-end results. Each stage first states the options we tried and then reports the corresponding scores, so the best path and its cost become clear before the case study.",
      ...
      "world.p.models": "Generation and evaluation roles are separated as follows. Qwen3.6-27B predicts boundaries and can also generate label candidates. Qwen3.5-397B-A17B generates candidate descriptions and serves as the candidate selector. Gemini-3.5-Flash is used only during offline evaluation to judge semantic agreement with the human annotation; it does not predict boundaries, generate descriptions, or select candidates.",
