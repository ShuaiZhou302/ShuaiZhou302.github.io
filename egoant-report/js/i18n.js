...
(function () {
  const I18N = {
    zh: {
      ...
      "world.p.rule": "<a href=\"https://microsoft.github.io/VITRA/\" target=\"_blank\" rel=\"noopener\">VITRA</a><sup class=\"cite\"><a class=\"cite-ref\" href=\"#ref-vitra\">5</a></sup> 将单目第一视角视频转换为三维手部与相机运动、原子动作片段和语言指令，并以腕部速度的局部极小值作为候选动作边界。另一类方法直接使用预设时长。在 Toyota Research Institute 的 <a href=\"https://co-training-lbm.github.io/\" target=\"_blank\" rel=\"noopener\"><em>A Systematic Study of Data Modalities and Strategies for Co-training Large Behavior Models for Robot Manipulation</em></a><sup class=\"cite\"><a class=\"cite-ref\" href=\"#ref-cotraining\">10</a></sup> 中，研究者构建人类视频语言标注数据时，沿用 <a href=\"https://egovid.github.io/\" target=\"_blank\" rel=\"noopener\">EgoVid</a> 的划分方式，将筛选后的 Ego4D 视频切成固定 4 秒片段，再按 1 秒间隔抽帧生成描述。两类方法都容易<strong>过分割</strong>：运动低谷会把动作内部的停顿、重新抓握和手腕微调误判为边界；固定时长切分则不考虑动作何时真正完成，可能在同一动作内部反复插入切点，使「把胡萝卜放进碗里」这样的简单原子动作也被拆成多个片段。本文探索了基于 HaWoR 腕速低谷的分段方法，并合并相邻片段；在 HomER 上，该方法预测 810 段，而人工参考为 470 段，<strong>分段得分为 0.0953</strong>，印证了过分割问题。一种应对方式是将连续抽帧拼成带时间戳的拼贴图，让模型从更完整的时间上下文中判断动作边界，从而减少不必要的切分。",
      ...
      "world.h3.egoant": "本文方法 EgoANT：面向开放权重模型的端到端自动标注",
      ...
      "world.gepa.1": "<strong>GEPA 本身</strong>是一种根据反馈自动改写提示词的方法<sup class=\"cite\"><a class=\"cite-ref\" href=\"#ref-gepa\">3</a></sup>。Macrodata Labs 用它在独立的 15 集验证集上搜索更贴合标注规范的分段规则。",
      "world.gepa.2": "<strong>本文复用的是搜索结果</strong>，即公开的英文分段规则 <code>completed_events_duration_prior_v1</code>；本文没有重新运行 GEPA。",
      "world.gepa.3": "<strong>使用方式</strong>：将完整视频的全部带时间戳拼贴图与规则文本一并提交，在一次模型调用中得到整集粗分结果。",
      "world.gepa.4": "规则原文见案例分析 Step 02 的折叠区；概念与实现见 <a href=\"#app-seg\">附录 B</a>，下载见 <a href=\"#app-prompts\">附录 F</a>。",
      "world.h3.terms": "视频分段常用术语",
      "world.terms.figcap": "图中按处理顺序展示：人工参考仅用于评测；模型先对完整视频做一次粗分，再选取一个局部时间窗口，并在窗口内执行 S2 局部精修。",
      "world.term.1": "<strong>参考分段（gold）</strong>：人工标注的完整动作片段，图中用黄色表示。它只用于评测，不会作为分段模型的输入。",
      "world.term.2": "<strong>整集粗分</strong>：将完整视频的全部带时间戳拼贴图一次性提交给模型，由一次模型调用输出初始分段序列。",
      "world.term.3": "<strong>粗分片段与粗边界</strong>：整集粗分输出的初始片段及其起止时间。它们是后续精修的初始估计，还不是最终边界。",
      "world.term.4": "<strong>局部时间窗口</strong>：围绕某个粗边界截取的一小段连续视频区间，图中用绿色虚线框表示。模型会重新检查这段区间内的动作起止。",
      "world.term.5": "<strong>S1（加密切分配置）</strong>：本文对一类消融配置的简称。它仍对完整视频做一次分段，但要求输出更多、更短的候选片段；它不是最终管线中的处理步骤。实验结果见 <a href=\"#story\">§7.1</a>。",
      "world.term.6": "<strong>S2（局部精修配置）</strong>：本文对第二遍局部分段配置的简称。先由整集粗分得到粗边界，再围绕粗边界建立时间窗口，并让模型在窗口内重新输出动作边界。",
      "world.term.7": "<strong>窗口不外扩（pad=0）</strong>：局部精修只使用时间窗口内的画面，不额外加入窗口之前或之后的帧。",
      "world.term.8": "<strong>覆盖完整动作（full-cover）</strong>：S2 局部精修提示词中的输出约束，不是评分指标。它要求模型完整覆盖窗口内从开始到结束都可见的操作事件；例如窗口内完整出现“拿起杯子”和“放下杯子”时，应分别输出两段。",
      "world.term.9": "<strong>候选判别器（selector）</strong>：在同一预测边界上有多条候选描述时，从中选出最终描述；只在描述生成阶段使用。",
      "world.term.10": "<strong>语义评判模型（judge）</strong>：只在评测阶段使用，判断预测描述与人工描述是否表达同一个完成动作。",
      "world.term.11": "<strong>HaWoR</strong><sup class=\"cite\"><a class=\"cite-ref\" href=\"#ref-hawor\">6</a></sup>：从第一视角视频中重建手部运动的方法，本文用它估计腕部轨迹并裁出手部区域。",
      "fig.src.segterms": "assets/explain/seg_terms_zh.svg?v=20260810-2",
      ...
      "world.p.inputs": "后续实验使用六类视觉输入：原始帧、时序拼贴图、带时间戳的拼贴图、相邻片段拼贴图、视觉提示叠加和手部区域裁剪。下图使用同一视频片段（homer_4 第 22.9–26.8 秒，“拉开床头柜抽屉”）展示它们在图像组织方式和上下文范围上的差别。",
      "world.zoo.raw": "<strong>原始帧</strong>：在目标片段内均匀抽取 3 帧，以 3 张独立图片的形式提交给模型。",
      "world.zoo.collage": "<strong>时序拼贴图</strong>：将目标片段内按时间顺序抽取的多帧图像拼成一张网格图，再作为单张图片提交。",
      "world.zoo.contact": "<strong>带时间戳的拼贴图</strong>：每 0.5 秒抽取一帧，缩放至 224×144，每 20 帧组成一张网格图，并在每格左上角标注黄色时间戳；一张图约覆盖 10 秒。",
      "world.zoo.neighbor": "<strong>相邻片段拼贴图</strong>：同时排列前一片段、当前片段和后一片段的抽样帧，并用底部色条标出三者范围。",
      "world.zoo.proxy": "<strong>视觉提示叠加</strong>：在原始画面上增加用于引导注意区域的视觉标记。具体提示生成方式将在后续消融部分进一步核对。",
      "world.zoo.crop": "<strong>手部区域裁剪</strong>：根据视觉提示选取局部区域，仅将裁剪后的图像提交给模型；它与在完整画面上保留提示标记是两种不同输入。",
      "world.zoo.note": "以上示例仅说明六类输入的组织形式；对应实验结果见 <a href=\"#story\">§7.2</a>。",
      ...
      "metrics.lead": "三种分数依次衡量：<strong>时间边界是否准确</strong>、<strong>给定边界后的描述是否正确</strong>，以及<strong>端到端分段与标注的整体表现</strong>。公式汇总见 <a href=\"#app-metrics\">附录 A</a>。",
    ...
    en: {
      ...
      "world.h3.egoant": "Our approach, EgoANT: end-to-end annotation with open-weight models",
      ...
      "world.gepa.1": "<strong>GEPA itself</strong> automatically rewrites a prompt from feedback<sup class=\"cite\"><a class=\"cite-ref\" href=\"#ref-gepa\">3</a></sup>. Macrodata Labs used it on a separate 15-episode validation set to search for segmentation rules that better follow the annotation protocol.",
      "world.gepa.2": "<strong>This report reuses the search result</strong>: the published English rule text <code>completed_events_duration_prior_v1</code>. We did not rerun GEPA.",
      "world.gepa.3": "<strong>How it is used</strong>: all timestamped grids for the complete video are submitted together with the rule text, producing the whole-episode coarse segmentation in one model call.",
      "world.gepa.4": "The full rules appear in the Step 02 fold of the case study; concepts and implementation are in <a href=\"#app-seg\">Appendix B</a>, with the downloadable prompt in <a href=\"#app-prompts\">Appendix F</a>.",
      "world.h3.terms": "Common video-segmentation terms",
      "world.terms.figcap": "The diagram follows the processing order: the human reference is used only for evaluation; the model first performs one whole-episode coarse pass, selects a local time window, and then applies S2 local refinement inside that window.",
      "world.term.1": "<strong>Reference segments (gold)</strong>: complete actions annotated by humans and shown in yellow. They are used only for evaluation, not as input to the segmentation model.",
      "world.term.2": "<strong>Whole-episode coarse pass</strong>: all timestamped grids for the complete video are sent together, and one model call outputs the initial segment sequence.",
      "world.term.3": "<strong>Coarse segments and coarse boundaries</strong>: the initial segments and timestamps returned by the whole-episode pass. They seed refinement but are not yet the final boundaries.",
      "world.term.4": "<strong>Local time window</strong>: a short continuous interval around one coarse boundary, shown by the dashed green box. The model re-examines action start and end times within this interval.",
      "world.term.5": "<strong>S1 (denser-cut configuration)</strong>: shorthand for an ablation that still segments the full video in one pass but asks for more, shorter candidate segments. It is not a stage in the final pipeline. Results are in <a href=\"#story\">§7.1</a>.",
      "world.term.6": "<strong>S2 (local-refinement configuration)</strong>: shorthand for the second-pass local segmentation setup. A whole-episode pass first produces coarse boundaries; a window is then built around each boundary, and the model predicts action boundaries again inside that window.",
      "world.term.7": "<strong>No pad-out (pad=0)</strong>: local refinement uses only frames inside the time window, without adding frames from before or after it.",
      "world.term.8": "<strong>Cover full actions (full-cover)</strong>: an output requirement in the S2 local-refinement prompt, not an evaluation metric. The model must cover every manipulation event whose start and completion are both visible in the window; if both \"pick up the cup\" and \"put down the cup\" are fully shown, it should output two segments.",
      "world.term.9": "<strong>Candidate selector</strong>: chooses the final description when several candidates exist for the same predicted boundary; used only during description generation.",
      "world.term.10": "<strong>Semantic judge</strong>: used only during evaluation to decide whether the predicted and human descriptions express the same completed action.",
      "world.term.11": "<strong>HaWoR</strong><sup class=\"cite\"><a class=\"cite-ref\" href=\"#ref-hawor\">6</a></sup>: reconstructs hand motion from egocentric video; this report uses it to estimate wrist trajectories and crop hand regions.",
      "fig.src.segterms": "assets/explain/seg_terms_en.svg?v=20260810-2",
      ...
      "world.p.inputs": "Later experiments use six visual-input formats: raw frames, a temporal collage, timestamped contact sheets, a neighboring-segment collage, visual-hint overlays, and hand-region crops. The examples below use the same span from homer_4 (22.9&ndash;26.8s, \"open the nightstand drawer\") to isolate differences in image organization and temporal context.",
      ...
      "world.zoo.collage": "<strong>Temporal collage</strong>: multiple frames sampled in temporal order from the target segment and tiled into one grid image.",
      "world.zoo.contact": "<strong>Timestamped contact sheet</strong>: one frame every 0.5s, scaled to 224&times;144, with 20 frames per grid and a yellow timestamp in each tile; one sheet covers about 10 seconds.",
      "world.zoo.neighbor": "<strong>Neighboring-segment collage</strong>: sampled frames from the previous, current, and next segments are arranged together, with colored bars indicating their ranges.",
      "world.zoo.proxy": "<strong>Visual-hint overlay</strong>: a visual marker is added to the full frame to guide attention toward a region. The exact hint-generation method will be checked separately in the later ablation discussion.",
      "world.zoo.crop": "<strong>Hand-region crop</strong>: a local region is selected from the visual hint and only the cropped image is sent to the model; this is distinct from retaining the hint on the full frame.",
      "world.zoo.note": "These examples show only how the six inputs are organized; corresponding results appear in <a href=\"#story\">&sect;7.2</a>.",
      ...
      "metrics.lead": "The three scores evaluate progressively broader capabilities: <strong>temporal boundary accuracy</strong>, <strong>description quality when boundaries are given</strong>, and <strong>the overall end-to-end performance of the segmentation-and-labeling pipeline</strong>. See <a href=\"#app-metrics\">Appendix A</a> for the formulas.",
