...
(function () {
  const I18N = {
    zh: {
      ...
      "intro.why": "原始视频本身也可用于视频预测或世界模型训练；若结合手部重建得到的腕部轨迹，还可以提供弱动作监督。然而，这些信号通常不显式说明<strong>当前正在执行哪项操作，以及该操作何时完成</strong>。",
      "intro.task": "这类时间对齐的高层语义正是许多机器人策略所需要的监督。VLA 训练样本通常由「观测、语言指令、动作」组成，其中语言指令必须与相应的时间区间对齐。π<sub>0.5</sub> 等层级模型还会先预测高层子任务指令，再以此为条件生成低层动作<sup class=\"cite\"><a class=\"cite-ref\" href=\"#ref-pi05\">1</a></sup>；训练这一高层模块需要时间对齐的子任务标签。分段边界同样具有独立价值：它界定一次完整操作的起止，可作为成功判定、奖励建模、技能检索以及预训练数据筛选与配比的基本单位。因此，将长视频转化为可用监督，需要同时回答两个问题：<strong>每项操作何时开始、何时结束</strong>，以及<strong>该时间段内完成了什么操作</strong>。本文将这两个问题分别称为子任务分段与语义标注。",
      "intro.hard": "自动完成这两项任务并不容易。第一视角画面随头部运动，手部经常遮挡操作对象；相邻动作之间通常缺少清晰停顿，细粒度操作又可能连续发生。逐段人工标注难以扩展到千小时规模，因此需要一套<strong>可审计、可复现且成本可控的自动标注流程</strong>。",
      ...
      "tldr.pt3": "<strong>contact sheet 的主要收益体现在分段，而非固定边界标注。</strong>时间戳网格能够在单次调用中呈现较长时间范围内的动作变化，有利于判断操作何时完成。边界已经给定时，标注模型更依赖片段内部的手部与物体细节；加入相邻时间上下文并未提高准确率，错误分析还发现相邻动作有时会被误写入当前描述。因此，分段与标注需要采用不同的视觉输入设计。",
      "tldr.pt4": "<strong>当前端到端性能首先受时间边界限制。</strong>最佳分段配置在 308 个预测片段与 470 个参考片段之间得到 79 个时间匹配，Segment F1 为 0.2031；selector 路径在这些匹配中通过 60 个语义判断，最终 Semantic E2E F1 为 0.1542。在相同预测边界和匹配规则下，E2E F1 不可能超过 Segment F1。由此看，现阶段提高时间匹配数的潜在收益高于仅继续优化描述文本。",
      "tldr.pt5": "<strong>多候选筛选取得最高 E2E 分数，但提升伴随更高推理成本。</strong>在相同预测边界上，分段模型自带描述的 E2E F1 为 0.1234，单路重新标注为 0.1285–0.1491，多候选加 Qwen3.5-397B selector 则达到 <strong>0.1542</strong>。按当前公开单价和本文的 token／输出长度假设估算，单路 raw 路径约为 <strong>$0.84–$1.59 / 视频小时</strong>，selector 路径约为 <strong>$2.11–$3.80 / 视频小时</strong>；两者均不含离线 Gemini judge 费用。估算口径见 <a href=\"#cost\">§5</a>。",
    ...
    en: {
      ...
      "intro.ego": "Egocentric human video is typically captured by a head-mounted camera as a person performs everyday manipulation in a real environment: chopping vegetables in a kitchen, assembling parts at a workbench, or restocking shelves in a store. The camera moves with the head, while the hands and manipulated objects dominate the view. Lighting changes, occlusions, failures, and retries occur naturally rather than in a staged laboratory setup. Public datasets including EgoVerse<sup class=\"cite\"><a class=\"cite-ref\" href=\"#ref-egoverse\">8</a></sup>, EgoDex<sup class=\"cite\"><a class=\"cite-ref\" href=\"#ref-egodex\">7</a></sup>, and EgoLive<sup class=\"cite\"><a class=\"cite-ref\" href=\"#ref-egolive\">9</a></sup> have expanded this type of footage to the thousand-hour scale.",
      "intro.value": "The potential value of this data for robot learning begins with the <strong>transferability of manipulation semantics</strong>. Human hands and robot end-effectors differ in morphology, but object states, task goals, and the high-level relation between observation and action remain partly shared. Human video can therefore support pretraining for vision-language-action (VLA) models, followed by on-robot data that adapts the action space to a particular embodiment<sup class=\"cite\"><a class=\"cite-ref\" href=\"#ref-pi05\">1</a></sup>. Compared with teleoperated robot data, real human video also offers broader coverage of scenes, objects, and skills, and can be collected in parallel without robot hardware or teleoperators.",
      "intro.why": "Raw video can already train video-prediction or world models; paired with wrist trajectories from hand reconstruction, it can also provide weak action supervision. These signals, however, do not explicitly identify <strong>which operation is underway or when that operation is complete</strong>.",
      "intro.task": "Many robot policies require precisely this time-aligned, high-level supervision. A VLA training sample commonly combines an observation, a language instruction, and an action, with the instruction aligned to the relevant time interval. Hierarchical models such as &pi;<sub>0.5</sub> first predict a high-level subtask instruction and then condition low-level actions on it<sup class=\"cite\"><a class=\"cite-ref\" href=\"#ref-pi05\">1</a></sup>; training that high-level module requires time-aligned subtask labels. Segment boundaries are useful in their own right: they define the start and completion of an operation and provide units for success detection, reward modeling, skill retrieval, and corpus filtering. Turning long video into usable supervision therefore requires two outputs: <strong>the start and end of each operation</strong>, and <strong>a semantic description of what was completed in that interval</strong>. This report refers to them as subtask segmentation and semantic labeling.",
      "intro.hard": "Automating both outputs is difficult. Egocentric footage moves with the wearer's head, hands often occlude the manipulated object, adjacent actions rarely have clean pauses, and fine-grained operations may unfold continuously. Manual segment-level annotation does not readily scale to thousand-hour corpora, motivating an <strong>auditable, reproducible, and cost-controlled annotation pipeline</strong>.",
      ...
      "tldr.pt3": "<strong>The main benefit of contact sheets appears in segmentation, not fixed-boundary labeling.</strong> Timestamped grids expose action changes over a longer interval in one request, which helps identify when an operation completes. Once boundaries are fixed, labeling depends more on hand and object detail inside the segment. Adding neighboring temporal context did not improve accuracy, and error analysis found cases where an adjacent action leaked into the current description. Segmentation and labeling therefore benefit from different visual-input designs.",
      "tldr.pt4": "<strong>Temporal boundaries are the first constraint on current end-to-end performance.</strong> The best segmentation setup produces 79 temporal matches between 308 predictions and 470 references, for Segment F1 0.2031. The selector path passes the semantic judge on 60 of those matches, resulting in Semantic E2E F1 0.1542. Under the same predicted boundaries and matching rule, E2E F1 cannot exceed Segment F1. Increasing the number of temporal matches therefore offers more headroom than refining descriptions alone.",
      "tldr.pt5": "<strong>Multi-candidate selection achieves the best E2E score, but at a higher inference cost.</strong> On the same predicted boundaries, segmenter-written labels reach E2E F1 0.1234, single-pass relabeling reaches 0.1285&ndash;0.1491, and multiple candidates plus a Qwen3.5-397B selector reach <strong>0.1542</strong>. Using the supplied public prices and this report's token and output-length assumptions, the raw-only path is estimated at <strong>$0.84&ndash;$1.59 per video hour</strong>, versus <strong>$2.11&ndash;$3.80 per video hour</strong> for the selector path, excluding the offline Gemini judge. See <a href=\"#cost\">&sect;5</a> for assumptions.",
