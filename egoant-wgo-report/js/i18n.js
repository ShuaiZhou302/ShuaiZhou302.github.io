...
(function () {
  const I18N = {
    zh: {
      ...
      "story.takeaway": "<strong>要点：</strong>分片 contact sheet 易在接缝造假切点；切段规则定「切什么」、局部精修（盖住完整动作）定「切多细」；规则 merge 改善观感但没有提升 F1；Gemini 重评后，raw 27B 是固定边界标注最高分，但端到端最高仍是 S2 边界 + 397B 多候选 selector。换句话说，<strong>27B 是最好的 fixed-boundary captioner，但不是最好的 noisy predicted-segment label resolver</strong>。overlay、邻段和粗糙 hand-collage 目前都不该默认启用。",
      "th.cond": "条件",
      "th.model": "模型或方法",
      "th.note": "结果一句话",
      "th.acc": "Acc",
    ...
    en: {
      ...
      "story.takeaway": "<strong>Takeaways:</strong> chunked contact sheets invent seam cuts; the rule list decides what to cut, local refine (cover full actions) decides how fine; rule merges look nicer but don’t lift F1. After the Gemini rescore, raw 27B is the best fixed-boundary labeler, but the best end-to-end recipe is still S2 bounds + 397B multi-candidate selector. In short, <strong>27B is the best fixed-boundary captioner, but not the best noisy predicted-segment label resolver</strong>. Overlay, neighbor sheets, and crude hand-collages should not be default paths.",
      "th.cond": "Condition",
